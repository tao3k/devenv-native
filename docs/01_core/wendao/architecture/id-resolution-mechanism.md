...
2. **Embedded Path (Pure AST)**: Used by internal Rust crates to resolve built-in IDs from local `resources/` without database overhead. See [[docs/01_core/memory/injection-evolution.md|Rust Context Injection + Memory Self-Evolution]].
...
By using this ID-resolution mechanism, the Rust context-injection configuration bridge becomes practically instantaneous.

When a context-injection consumer asks for a template by ID:
