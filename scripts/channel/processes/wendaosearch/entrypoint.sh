...
PACKAGE_LABEL="${WENDAOSEARCH_PACKAGE_LABEL:-WendaoSearch.jl checkout}"
...
JULIA_PROJECT="$(process_abs_path "$PROJECT_ROOT" "$JULIA_PROJECT")"

source "$PROJECT_ROOT/scripts/channel/process-runtime.sh"
mkdir -p "$RUNTIME_DIR" "$LOG_DIR"
managed_materialize_git_repo "$PACKAGE_DIR" "$PACKAGE_REPO_URL" "" "$PACKAGE_LABEL"

SCRIPT_PATH="$(process_abs_path "$PACKAGE_DIR/scripts" "$SCRIPT_NAME")"
if [ ! -f "$SCRIPT_PATH" ]; then
  echo "Error: Julia service script not found: $SCRIPT_PATH" >&2
  exit 1
fi
if [ ! -f "$CONFIG_PATH" ]; then
  echo "Error: Julia service config not found: $CONFIG_PATH" >&2
  exit 1
fi
...
"$JULIA_BIN" --project="$JULIA_PROJECT" "$SCRIPT_PATH" --config "$CONFIG_PATH" \
  > >(tee -a "$STDOUT_LOG") \
