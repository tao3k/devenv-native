...
def test_valkey_status_reports_owned_listener(tmp_path: Path) -> None:
    project_root = Path(__file__).resolve().parents[2]
    script_path = Path(__file__).resolve().with_name("valkey-status.sh")
    ...
    assert "Valkey is running on 6387 (pid 12345)." in result.stdout
    assert "PONG" in result.stdout


def test_valkey_status_rejects_reachable_listener_without_matching_pidfile(
    tmp_path: Path,
) -> None:
    project_root = Path(__file__).resolve().parents[2]
    script_path = Path(__file__).resolve().with_name("valkey-status.sh")
...
def test_valkey_status_accepts_shared_pidfile(tmp_path: Path) -> None:
    project_root = Path(__file__).resolve().parents[2]
    script_path = Path(__file__).resolve().with_name("valkey-status.sh")
    fake_bin = _install_fake_valkey_cli(tmp_path)
