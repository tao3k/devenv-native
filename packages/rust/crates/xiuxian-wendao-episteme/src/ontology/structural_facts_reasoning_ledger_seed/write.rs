...
pub(super) fn write_seed_tsv(
    ...
    for row in rows {
        writeln!(
            ...
            escape_tsv(&row.seed_kind),
            escape_tsv(&row.packet_id),
            escape_tsv(&row.reasoning_task_kind),
            escape_tsv(&row.evidence_target_intent),
            escape_tsv(&row.evidence_anchor_kind),
            escape_tsv(&row.evidence_structure_hint),
...
pub(super) fn write_seed_org(
    path: &Path,
    report: &EpistemeOntologyStructuralFactsReasoningLedgerSeedReport,
    rows: &[EpistemeOntologyStructuralFactsReasoningLedgerSeedRow],
) -> Result<()> {
    let mut file = create_file(path)?;
    writeln!(
        file,
        "#+TITLE: Episteme Structural Facts Reasoning Ledger Seed"
    )?;
    writeln!(file)?;
    ...
    writeln!(
        file,
        ":WENDAO_KIND: episteme_structural_facts_reasoning_ledger_seed"
    )?;
    ...
    writeln!(
        ...
        report.service_catalog_seed_row_count
    )?;
    writeln!(
        file,
        "| object_instance_seed_row_count | {} |",
        report.object_instance_seed_row_count
    )?;
    ...
    write_structured_review_seed_table(&mut file, rows)?;
    Ok(())
}
...
fn write_object_seed_table(
    ...
    writeln!(
        file,
        "| seed_id | packet_id | target_intent | structure_hint | evidence_id | document_id | anchor | file_id | proposed_object_id | proposed_object_type | proposed_label | review_decision | promotion_decision | reviewer_id |"
    )?;
    writeln!(file, "|-|-|-|-|-|-|-|-|-|-|-|-|-|-|")?;
    for row in rows
        .iter()
        .filter(|row| row.seed_kind == "object_proposal_slot")
    {
        writeln!(
            ...
            org_cell(&row.evidence_target_intent),
            org_cell(&row.evidence_structure_hint),
...
fn write_relation_seed_table(
    ...
    writeln!(
        file,
        "| seed_id | packet_id | target_intent | structure_hint | evidence_id | document_id | anchor | file_id | proposed_relation_id | proposed_source_object_id | proposed_predicate | proposed_target_object_id | review_decision | promotion_decision | reviewer_id |"
    )?;
    writeln!(file, "|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|")?;
    for row in rows
        .iter()
...
fn write_structured_review_seed_table(
    file: &mut File,
    rows: &[EpistemeOntologyStructuralFactsReasoningLedgerSeedRow],
) -> Result<()> {
    let review_rows = rows.iter().filter(|row| {
        matches!(
            row.seed_kind.as_str(),
            "service_catalog_review_slot" | "object_instance_review_slot"
        )
    });
    writeln!(file)?;
    writeln!(file, "** Structure-targeted review slots")?;
    writeln!(file)?;
    writeln!(
        file,
        "| seed_id | seed_kind | packet_id | target_intent | structure_hint | evidence_id | document_id | anchor | file_id | review_decision | promotion_decision | reviewer_id |"
    )?;
    writeln!(file, "|-|-|-|-|-|-|-|-|-|-|-|-|")?;
