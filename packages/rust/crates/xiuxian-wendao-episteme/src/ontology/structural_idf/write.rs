...
use super::types::{
    ...
    EpistemeOntologyStructuralIdfRelationRow, EpistemeOntologyStructuralIdfReport,
};

pub(super) fn write_documents_tsv(
    path: &Path,
    rows: &[EpistemeOntologyStructuralIdfDocumentRow],
) -> Result<()> {
...
pub(super) fn write_anchors_tsv(
    path: &Path,
    rows: &[EpistemeOntologyStructuralIdfAnchorRow],
) -> Result<()> {
    let mut file = create_file(path)?;
    writeln!(
        file,
...
pub(super) fn write_relations_tsv(
    path: &Path,
    rows: &[EpistemeOntologyStructuralIdfRelationRow],
) -> Result<()> {
    let mut file = create_file(path)?;
    writeln!(
        file,
...
pub(super) fn write_structural_idf_org(
    path: &Path,
    report: &EpistemeOntologyStructuralIdfReport,
) -> Result<()> {
    let mut file = create_file(path)?;
    writeln!(file, "#+TITLE: Episteme Structural IDF Seed")?;
    writeln!(file)?;
    writeln!(file, "* Structural IDF seed")?;
    writeln!(file, ":PROPERTIES:")?;
    writeln!(file, ":WENDAO_KIND: episteme_structural_idf_seed")?;
