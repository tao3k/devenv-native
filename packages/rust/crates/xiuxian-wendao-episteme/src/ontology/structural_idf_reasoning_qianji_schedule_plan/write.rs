...
pub(super) fn write_schedule_plan_tsv(
    ...
    writeln!(
        file,
        "schedule_item_id\tschedule_contract\tadmission_kind\tqianji_run_id\tfill_item_id\tworkflow_key\tactivity_kind\tseed_id\tpacket_id\tdocument_id\tdocument_anchor_id\tfile_id\tevidence_id\tfield_group\tactivity_id\tactivity_type\ttask_queue\tinput_artifact_id\tinput_artifact_kind\tidempotency_key\tsource_text_read\tllm_executed\tworkflow_executed\tqianji_ledger_mutated\thot_state_enqueued\tsource_mutation_allowed\trdf_mutation_allowed\tontology_truth\tstatus"
    )?;
    for item in items {
        writeln!(
            file,
...
pub(super) fn write_schedule_plan_org(
    path: &Path,
    report: &EpistemeOntologyStructuralIdfReasoningQianjiSchedulePlanReport,
    items: &[EpistemeOntologyStructuralIdfReasoningQianjiSchedulePlanItem],
) -> Result<()> {
    let mut file = create_file(path)?;
...
fn write_schedule_item_table(
    ...
    writeln!(
        file,
        "| schedule_item_id | qianji_run_id | fill_item_id | activity_id | activity_type | task_queue | evidence_id | field_group | status |"
    )?;
    writeln!(file, "|-|-|-|-|-|-|-|-|-|")?;
    for item in items {
        writeln!(
