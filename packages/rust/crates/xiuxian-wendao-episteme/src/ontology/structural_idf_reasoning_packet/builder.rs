...
use super::{
    input::{StructuralIdfAnchorInput, StructuralIdfDocumentInput, read_structural_idf_input},
    types::{
...
pub(super) struct ReasoningPacketBuild {
    pub rows: Vec<EpistemeOntologyStructuralIdfReasoningPacketRow>,
...
pub(super) fn build_reasoning_packet_rows(
    ...
    let input = read_structural_idf_input(request.structural_idf_json.as_path())?;
...
fn document_root_anchors(
    anchors: &[StructuralIdfAnchorInput],
) -> Result<BTreeMap<&str, &StructuralIdfAnchorInput>> {
        ...
        .filter(|anchor| anchor.anchor_kind == "document_root")
    {
        ...
        {
            bail!(
                "structural IDF input has duplicate document_root anchors for `{}`",
                anchor.document_id
            );
        }
    }
...
fn validate_document(
    document: &StructuralIdfDocumentInput,
    document_anchors: &BTreeMap<&str, &StructuralIdfAnchorInput>,
) -> Result<()> {
...
fn packet_row(
    document: &StructuralIdfDocumentInput,
    anchor: &StructuralIdfAnchorInput,
    run_id: &str,
) -> EpistemeOntologyStructuralIdfReasoningPacketRow {
    EpistemeOntologyStructuralIdfReasoningPacketRow {
...
fn stable_packet_id(run_id: &str, document_id: &str, anchor_id: &str) -> String {
    ...
    format!("idf.reasoning_packet.{suffix}")
}
