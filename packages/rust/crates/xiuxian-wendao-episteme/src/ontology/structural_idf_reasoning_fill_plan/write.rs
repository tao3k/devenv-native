...
pub(super) fn write_fill_plan_org(
    path: &Path,
    report: &EpistemeOntologyStructuralIdfReasoningFillPlanReport,
    items: &[EpistemeOntologyStructuralIdfReasoningFillPlanItem],
) -> Result<()> {
    let mut file = create_file(path)?;
    writeln!(file, "#+TITLE: Episteme Structural IDF Reasoning Fill Plan")?;
    writeln!(file)?;
    writeln!(file, "* Reasoning fill plan")?;
    writeln!(file, ":PROPERTIES:")?;
    writeln!(
        file,
        ":WENDAO_KIND: episteme_structural_idf_reasoning_fill_plan"
    )?;
...
fn write_work_item_table(
    ...
    writeln!(
        file,
        "| fill_item_id | workflow_key | activity_kind | seed_id | seed_kind | packet_id | document_id | anchor | file_id | field_group | status |"
    )?;
    writeln!(file, "|-|-|-|-|-|-|-|-|-|-|-|")?;
    for item in items {
        writeln!(
