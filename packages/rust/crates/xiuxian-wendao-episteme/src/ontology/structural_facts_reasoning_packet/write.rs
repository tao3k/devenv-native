...
pub(super) fn write_packet_tsv(
    path: &Path,
    rows: &[EpistemeOntologyStructuralFactsReasoningPacketRow],
) -> Result<()> {
    let mut file = create_file(path)?;
    writeln!(
        file,
        "packet_id\tpacket_kind\treasoning_task_kind\tevidence_target_intent\tevidence_anchor_kind\tevidence_structure_hint\tdocument_id\tdocument_anchor_id\tfile_id\tdomain_id\tsource_contract_id\trelative_path\tcategory\tlanguage\textraction_route\tsource_content_hash\tevidence_action\tontology_truth\tstatus"
    )?;
    for row in rows {
        writeln!(
            file,
            ...
            row.evidence_target_intent,
            escape_tsv(&row.evidence_anchor_kind),
            row.evidence_structure_hint,
...
pub(super) fn write_packet_org(
    path: &Path,
    report: &EpistemeOntologyStructuralFactsReasoningPacketReport,
    rows: &[EpistemeOntologyStructuralFactsReasoningPacketRow],
) -> Result<()> {
    let mut file = create_file(path)?;
    writeln!(file, "#+TITLE: Episteme Structural Facts Reasoning Packet")?;
    writeln!(file)?;
    writeln!(file, "* Reasoning packet")?;
    writeln!(file, ":PROPERTIES:")?;
    writeln!(
        file,
        ":WENDAO_KIND: episteme_structural_facts_reasoning_packet"
    )?;
    ...
    writeln!(
        file,
        "This packet is a bounded proposal input surface. It cites generated structural facts rows and does not contain raw private source text."
    )?;
    writeln!(file)?;
    ...
    writeln!(
        ...
        org_cell(&report.structural_facts_json.display().to_string())
    )?;
    ...
    writeln!(
        file,
        "| packet_id | task | target_intent | structure_hint | file_id | relative_path | category | route | anchor | hash |"
    )?;
    writeln!(file, "|-|-|-|-|-|-|-|-|-|-|")?;
    for row in rows {
        writeln!(
