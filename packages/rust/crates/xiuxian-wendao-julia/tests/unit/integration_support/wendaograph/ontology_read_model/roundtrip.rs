...
use super::support::{parent_registry_batches, sample_request_batches};
use super::{
...
#[tokio::test]
async fn ontology_extension_proof_roundtrip_uses_runtime_flight_exchange() {
    let (base_url, server) = spawn_ontology_quality_flight_service().await;
    let binding = build_wendaograph_ontology_read_model_quality_flight_binding(
        WendaoGraphOntologyReadModelQualityFlightBindingOptions {
            base_url,
    ...
    .unwrap_or_else(|error| panic!("build ontology extension proof binding: {error}"));
    ...
    .await
    .unwrap_or_else(|error| panic!("ontology extension proof roundtrip should succeed: {error:?}"))
    .unwrap_or_else(|| panic!("binding should negotiate a runtime transport"));
