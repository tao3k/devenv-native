    ...
    (0..batch.num_rows())
        .map(|row_index| {
            Ok(SearchStrategyFlowServiceCandidateRow {
                candidate_id: string_value(batch, "candidate_id", row_index)?,
    ...
    (0..batch.num_rows())
        .map(|row_index| {
            Ok(SearchStrategyFlowFrontierRow {
                flow_id: string_value(batch, "flow_id", row_index)?,
                frontier_id: string_value(batch, "frontier_id", row_index)?,
                candidate_id: string_value(batch, "candidate_id", row_index)?,
                revision_id: string_value(batch, "revision_id", row_index)?,
                ...
                judgement_kind: string_value(batch, "judgement_kind", row_index)?,
            })
        })
        .collect()
}
    ...
    (0..batch.num_rows())
        .map(|row_index| {
            Ok(SearchStrategyFlowServicePlannerActionRow {
                action_kind: string_value(batch, "action_kind", row_index)?,
                candidate_id: string_value(batch, "candidate_id", row_index)?,
                target_candidate_id: string_value(batch, "target_candidate_id", row_index)?,
