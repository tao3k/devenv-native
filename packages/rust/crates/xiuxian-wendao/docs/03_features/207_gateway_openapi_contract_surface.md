...
Gateway is the only public edge for user API tokens. It exposes two public
protocol surfaces with the same bearer-token semantics:

- `gateway-https-v1`: HTTPS JSON/SSE routes described by the checked-in
  OpenAPI artifact.
- `gateway-flight-v1`: the same-listener Arrow Flight route mounted at
  `/arrow.flight.protocol.FlightService/{*grpc_method}`.

External HTTPS callers authenticate with a bearer token and send ordinary JSON
requests:
...
Gateway Arrow Flight callers use the same bearer token in request metadata or
the `Authorization` header. Authentication is enforced on the Flight route
independently of the HTTPS JSON route layer, so enabling bearer auth protects
both public protocol surfaces while `/api/health` remains unauthenticated.
FlightSQL, standalone search Flight hosts, and non-gateway gRPC-Web surfaces
remain deployment and operator controls outside this public Gateway edge.
...
or `stream: true` returns server-sent response events. The checked-in OpenAPI
artifact owns the HTTPS request and response schemas for this route under
`GatewayResponseRequest`, `GatewayResponse`, and `GatewayResponseOutput`.

### Gateway Flight Runtime Controls

The gateway can mount the Studio HTTP router and the Gateway Arrow Flight route
on the same listener, but the two branches have independent Tower budgets and
independent bearer enforcement. Operators should treat the following
environment variables as the stable runtime contract for public-gateway
authentication plus gateway and search-plane Flight compatibility surfaces.

| Environment variable                                 | Default                                                       | Operational use                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `XIUXIAN_WENDAO_GATEWAY_BEARER_TOKEN`                | unset                                                         | Enables bearer-token enforcement for public gateway HTTPS JSON/SSE routes and the same-listener Gateway Arrow Flight route when set to a non-empty value. `/api/health` remains unauthenticated for readiness probes. Callers send `Authorization: Bearer <value>`, for example `Authorization: Bearer wd_...`. |
