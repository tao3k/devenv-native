...
| Many unique cold documents                                        | One instance cannot absorb 10000 unique OCR-heavy cold misses quickly                   | Default Rust conversion permits to host parallelism, use `WENDAO_DOCUMENT_EXTRACT_ENDPOINTS` for Python worker pools, cap with `WENDAO_DOCUMENT_EXTRACT_MAX_RUNNING_CONVERSIONS` per deployment, add horizontal workers, and keep fast paths behind parity gates |
