...
- [DONE] markdown semantic frontmatter parsing now lives under `src/parsers/markdown/frontmatter/`, the duplicate `enhancer/frontmatter.rs` implementation is retired, and enhancement plus `skill_vfs` discovery flows now consume `crate::parsers::markdown::{parse_frontmatter, NoteFrontmatter}` with parser-owned unit coverage.
...
- [DONE] zhixing task-line parsing now lives under `src/parsers/zhixing/tasks/`, the former `skill_vfs::zhixing::indexer::parse/` subtree is retired, and zhixing task ingestion plus agenda-status stats now consume `crate::parsers::zhixing::tasks::{parse_task_projection, normalize_identity_token, TaskLineProjection}` with parser-owned unit coverage.
...
- [DONE] `skill_vfs/internal_manifest/authority.rs` now consumes `src/parsers/markdown/references/` for ordinary `SKILL.md` manifest intents, so Markdown links and wikilinks follow the same parser-owned contract instead of a local mixed scanner.
