...
- All five Repo Intelligence query slices are now wired end to end:
  ...
  - the same slice now also lands that baseline in the embedded system config under `resources/config/wendao.toml`: Wendao's default `search.duckdb` profile now explicitly enables the bounded DuckDB lane with runtime-owned storage under `$PRJ_DATA_HOME/xiuxian-wendao/duckdb/`, `preserve_insertion_order=true`, and `parquet_metadata_cache=false`, while leaving unproven resource knobs such as `memory_limit` and `max_temp_directory_size` unset until a workload-specific benchmark justifies them. Gateway deployments can still override `search.duckdb.database_path` or `search.duckdb.temp_directory` through the root `wendao.toml`.
