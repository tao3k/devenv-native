...
- All five Repo Intelligence query slices are now wired end to end:
  ...
  - that evidence shifts the next `100000+` read-path focus away from Valkey recovery and away from a presumed Flight transport hop: the dominant current cost is the local DuckDB scan/filter plus Arrow-to-Rust result consumption, because the warmed query and the Flight-facing batch materialization stay in the same roughly `1s` band
