...
- **Landed**: Package-Local Docs Governance (v2.19).
  ...
  - that hardened fix path has already been exercised against real package-local docs: `xiuxian-llm/docs/index.md` and `xiuxian-wendao/docs/index.md` now self-remediate through `wendao fix` and both package indexes audit clean after the generated section-landings, section-links, relations blocks, footer fixes, and opaque top-level `:ID:` repairs are applied
