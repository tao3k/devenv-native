...
impl SkillRuntimeResolver {
    ...
    /// # Errors
    ///
    /// Returns [`SkillRuntimeError`] when URI resolution fails or content lookup fails.
    pub fn read_utf8(&self, uri: &str) -> Result<String, SkillRuntimeError> {
    ...
    ///
    /// Returns [`SkillRuntimeError`] when URI resolution fails or content lookup fails.
    pub fn read_semantic(&self, uri: &str) -> Result<Arc<str>, SkillRuntimeError> {
        ...
        Err(SkillRuntimeError::ResourceNotFound {
            semantic_name: parsed.semantic_name().to_string(),
    ...
    ///
    /// Returns [`SkillRuntimeError`] when URI resolution fails or content lookup fails.
    pub fn read_utf8_shared(&self, uri: &str) -> Result<Arc<str>, SkillRuntimeError> {
        self.read_semantic(uri)
    }
    ...
    fn read_local_semantic(
        ...
        parsed: &WendaoResourceUri,
        canonical_uri: &str,
    ) -> Result<Option<Arc<str>>, SkillRuntimeError> {
