...
/// Deduplicated manifest intents declared by root `SKILL.md` documents.
///
...
pub struct SkillIntentCatalog {
    /// Canonical manifest URIs declared by root `SKILL.md` documents.
    pub intended_manifests: Vec<String>,
}

impl SkillIntentCatalog {
    /// Build a reusable skill intent catalog from prebuilt link-graph indexes.
    #[must_use]
...
impl SkillRuntimeResolver {
    /// Collect manifest intents from root `SKILL.md` documents mounted by this resolver.
    ///
    /// # Errors
    ///
    ...
    pub fn collect_manifest_intents(&self) -> Result<SkillIntentCatalog> {
        let mut intended_manifests = BTreeSet::new();
        for root in self.runtime_roots() {
            let path: &Path = root.as_path();
            let index = LinkGraphIndex::build(path).map_err(|error| {
                anyhow!(
                    "build link graph index for runtime skill root `{}`: {error}",
                    root.display()
                )
            })?;
            extend_manifest_intents_from_index(&index, &mut intended_manifests);
        }
        Ok(SkillIntentCatalog {
            intended_manifests: intended_manifests.into_iter().collect(),
        })
    }
}
...
pub(crate) fn extend_manifest_intents_from_index(
    ...
    for doc in index
        ...
        .into_iter()
        .filter(|doc| is_runtime_skill_doc_path(doc.path.as_str()))
    {
...
fn is_runtime_skill_doc_path(path: &str) -> bool {
    let normalized = path.trim().replace('\\', "/");
    normalized.ends_with(&format!("/{SKILL_DOC_NAME}"))
        && normalized
            .split('/')
...
fn normalize_manifest_intent(raw_target: &str) -> Option<String> {
    ...
    let candidate_uri = if trimmed.contains("://") {
        ...
        format!("wendao://skills/{normalized}")
    };
