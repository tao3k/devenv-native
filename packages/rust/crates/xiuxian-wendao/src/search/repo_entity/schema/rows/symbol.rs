...
pub(crate) fn build_symbol_row(
    ...
    let hit = SearchHit {
        ...
        audit_status: symbol.audit_status.as_ref().map(ToString::to_string),
        verification_state: symbol.verification_state.as_ref().map(ToString::to_string),
        implicit_backlinks,
    ...
    Ok(RepoEntityRow {
        ...
        symbol_kind,
        module_id: symbol.module_id.as_ref().map(ToString::to_string),
