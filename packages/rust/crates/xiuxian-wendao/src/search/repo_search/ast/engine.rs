...
use super::language::{Lang, normalize_code_language_identifier, supported_ast_lang};
use super::provider::{
    CodePatternMatch, extract_code_pattern_matches, extract_code_structure_symbols,
...
pub fn repository_generic_ast_lang_for_path(
    ...
    let excluded_languages = excluded_ast_languages_for_repository(repository);
    supported_ast_lang(path, &excluded_languages).map(Lang::as_str)
}
...
fn push_normalized_ast_language(languages: &mut Vec<String>, language: &str) {
    ...
    let normalized = normalize_code_language_identifier(trimmed)
