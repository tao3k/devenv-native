...
use xiuxian_daochang::{OmegaFallbackPolicy, OmegaRiskLevel, OmegaRoute, OmegaToolTrustClass};
...
fn reflective_runtime_enforces_diagnose_plan_apply_order() {
    let mut runtime = TestReflectiveRuntime::default();
    assert!(
        runtime
            .transition(TestReflectiveRuntimeStage::Diagnose)
            .is_ok()
    );
    assert!(runtime.transition(TestReflectiveRuntimeStage::Plan).is_ok());
    assert!(
        runtime
            .transition(TestReflectiveRuntimeStage::Apply)
            .is_ok()
    );
    assert_eq!(runtime.stage(), Some(TestReflectiveRuntimeStage::Apply));
}
...
fn reflective_runtime_rejects_illegal_transition_with_explicit_error() {
    let mut runtime = TestReflectiveRuntime::default();
    let error = runtime
        .transition(TestReflectiveRuntimeStage::Plan)
        .expect_err("plan before diagnose must be rejected");
    assert_eq!(error.from, None);
    assert_eq!(error.to, TestReflectiveRuntimeStage::Plan);
    assert!(
        error
            .to_string()
...
fn derive_policy_hint_prefers_verification_after_error() {
    let reflection = test_build_turn_reflection(
        "react",
    ...
    let hint =
        test_derive_policy_hint(&reflection, 42).expect("error reflection should emit a hint");
...
fn derive_policy_hint_prefers_fast_path_for_stable_tool_free_turn() {
    ...
    let hint = test_derive_policy_hint(&reflection, 7).expect("stable turn should emit a hint");
...
fn reflective_runtime_long_horizon_quality_thresholds() {
    ...
    for (index, (outcome, tool_calls, assistant_message)) in turn_inputs.iter().enumerate() {
        ...
        let hint = test_derive_policy_hint(&reflection, (index + 1) as u64);
