...
use xiuxian_daochang::test_support::{
    ...
    append_memory_recall_snapshot_payload, record_memory_recall_snapshot, session_messages,
    snapshot_session_id,
};
use xiuxian_daochang::{Agent, AgentConfig, SessionStore};
...
async fn record_and_inspect_memory_recall_snapshot_roundtrip() -> Result<()> {
    ...
    record_memory_recall_snapshot(&agent, &session_id, first).await;
    record_memory_recall_snapshot(&agent, &session_id, second).await;
    ...
    let storage_messages = session_messages(&agent, &snapshot_session_id(&session_id)).await?;
    assert_eq!(
...
async fn inspect_memory_recall_snapshot_normalizes_unknown_embedding_source() -> Result<()> {
    ...
    append_memory_recall_snapshot_payload(&agent, &session_id, payload).await?;
...
async fn inspect_memory_recall_snapshot_ignores_invalid_payload() -> Result<()> {
    ...
    append_memory_recall_snapshot_payload(&agent, &session_id, "not-json".to_string()).await?;
...
async fn memory_recall_snapshot_is_shared_across_agent_instances_with_valkey() -> Result<()> {
    ...
    record_memory_recall_snapshot(&agent_a, &session_id, snapshot).await;
...
async fn record_memory_recall_snapshot_publishes_stream_event() -> Result<()> {
    ...
    record_memory_recall_snapshot(&agent, &session_id, snapshot).await;
