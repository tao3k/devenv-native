...
`xiuxian-julia-core` is the Julia core bridge crate for existing
`xiuxian-wendao` consumers. It consumes
[`xiuxian-julia-runtime`](../xiuxian-julia-runtime/README.md) for
runtime-facing adapters and
[`xiuxian-polyglot-orchestrator`](../xiuxian-polyglot-orchestrator/README.md)
for inert profile/catalog facts, cross-language scheduling projection, memory
profile identities, and relationship-search evidence.

The dependency direction is intentionally one-way:
`xiuxian-julia-core` plus `xiuxian-julia-runtime` depend on
...
- crate-wide strict clippy is green again under
  `direnv exec . cargo clippy -p xiuxian-julia-core --all-targets --all-features -- -D warnings`
...
- the targeted transport regression proof is green under
  `direnv exec . cargo test -p xiuxian-julia-core plugin::graph_structural_transport::tests::validate_graph_structural_request_batches_accepts_staged_shapes -- --exact --nocapture`
- the full crate test gate now completes again under
  `direnv exec . cargo test -p xiuxian-julia-core`; the current full pass is
...
- SearchStrategyFlow Flight candidate discovery now runs route-scoped authority
  ...
  truncating the Julia input batch. Package-owned `xiuxian-julia-core`
...
- The ontology read-model quality bridge now exposes
  ...
  `RUN_WENDAOGRAPH_ONTOLOGY_QUALITY_LIVE_LOOPBACK_TEST=1 cargo test -p xiuxian-julia-core --lib ontology_read_model_quality_live_loopback -- --nocapture`;
...
- the process-managed `WendaoSearch.jl` background service now also has one
  ...
  `RUN_PROCESS_MANAGED_WENDAOSEARCH_TEST=1 cargo test -p xiuxian-julia-core plugin::graph_structural_exchange::tests::fetch_graph_structural_solver_demo_rows_for_repository_against_process_managed_wendaosearch_service -- --exact --nocapture`,
...
- the self-spawn `WendaoSearch.jl --mode solver_demo` graph-structural seam
  ...
  `RUN_WENDAOSEARCH_GRAPH_STRUCTURAL_PERF_TEST=1 cargo test -p xiuxian-julia-core graph_structural_live_perf -- --nocapture`.
...
- the current full crate pass is also green with that opt-in background lane
  enabled:
  `direnv exec . env RUN_PROCESS_MANAGED_WENDAOSEARCH_TEST=1 cargo test -p xiuxian-julia-core`
...
- `xiuxian-julia-core` also owns the Julia parser-summary client seam for
...
- `xiuxian-julia-core` also owns the parser-rich symbol identity seam for
...
- `xiuxian-julia-core` also owns the parser-rich Julia docstring target seam,
...
- `xiuxian-julia-core` also owns the bounded projection from parser-rich
...
- `xiuxian-julia-core` also owns the parser-rich Modelica symbol attribute
...
- `xiuxian-julia-core` also owns the Rust-to-Julia
...
- The parser-summary boundary is Flight-only for the touched Julia cutover
  surface. `xiuxian-julia-core` does not keep a Rust-local
...
- `xiuxian-julia-core` also owns the runtime-level memory-family thin compat
...
- the canonical staged defaults in that lane now use `/memory/calibration` for
  ...
  `memory_gate_score`
- `xiuxian-julia-core` now also owns the plugin-side host-adapter helpers
...
- `xiuxian-julia-core` also owns the runtime-facing memory-family transport
...
- for
  ...
  Phase 3 readiness evidence, `xiuxian-julia-core` owns the Julia-side
...
- `xiuxian-julia-core` also owns the Julia profile schedule-plan projection
...
- `xiuxian-julia-core` also mirrors the WendaoGraph PageIndex reasoning
...
- the same PageIndex host entrypoint now has an opt-in real Julia process
  probe:
  `RUN_WENDAOGRAPH_PAGE_INDEX_HOST_PROBE_TEST=1 WENDAOGRAPH_PACKAGE_DIR=<WendaoGraph.jl checkout> cargo test -p xiuxian-julia-core --lib wendaograph_page_index_host_probe -- --nocapture`.
...
- the same relationship-search evidence projection has an opt-in real
  ...
  `RUN_WENDAOGRAPH_RELATIONSHIP_SEARCH_LIVE_PERF_TEST=1 WENDAOGRAPH_PACKAGE_DIR=<WendaoGraph.jl checkout> cargo test -p xiuxian-julia-core --lib wendaograph_relationship_search_live_perf -- --nocapture`.
...
- the synthetic relationship-search proof now also has an opt-in repeated-run
  stability mode:
  `RUN_WENDAOGRAPH_RELATIONSHIP_SEARCH_SYNTHETIC_STABILITY_TEST=1 WENDAO_GRAPH_LINK_GRAPH_HOST_PROBE_MODE=synthetic-large WENDAOGRAPH_PACKAGE_DIR=<WendaoGraph.jl checkout> cargo test -p xiuxian-julia-core --lib wendaograph_relationship_search_synthetic_stability -- --nocapture`.
...
- the WendaoGraph LinkGraph host-request entrypoint now has an opt-in real
  Julia process probe:
  `RUN_WENDAOGRAPH_LINK_GRAPH_HOST_PROBE_TEST=1 WENDAOGRAPH_PACKAGE_DIR=<WendaoGraph.jl checkout> cargo test -p xiuxian-julia-core --lib wendaograph_link_graph_host_probe -- --nocapture`.
...
- the WendaoGraph GNN reasoning surface now has a separate opt-in real Julia
  process probe:
  `RUN_WENDAOGRAPH_GNN_HOST_PROBE_TEST=1 WENDAOGRAPH_PACKAGE_DIR=<WendaoGraph.jl checkout> cargo test -p xiuxian-julia-core --lib wendaograph_gnn_host_probe -- --nocapture`.
...
- `xiuxian-julia-core` also owns the Julia-side warmup and thread-diagnostic
...
- `xiuxian-julia-core` also exposes the owner-side
...
- `xiuxian-julia-core` also owns the plugin-side memory-family composition
...
- `xiuxian-julia-core` owns the Julia Arrow rerank exchange seam only where it stays Julia-specific: repository plugin-option interpretation, remote fetch helpers, and plugin-local loopback tests.
- `xiuxian-julia-core` also owns Julia-specific graph-structural transport option parsing, route-kind dispatch defaults, and staged request or response validation for promoted structural-search downcalls.
- `xiuxian-julia-core` also owns manifest-driven graph-structural binding fallback, so graph-structural client construction can derive route bindings from the live Julia capability manifest when explicit graph-structural transport config is absent.
- `xiuxian-julia-core` also owns one grouped same-endpoint capability-manifest
...
- `xiuxian-julia-core` also owns the plugin-side proof that one live `WendaoSearch.jl` endpoint can advertise the capability manifest and immediately serve graph-structural downcalls discovered from that same manifest.
- `xiuxian-julia-core` also owns the plugin-side proof that the same live
...
- `xiuxian-julia-core` also owns Julia-specific graph-structural route names, draft schema-version defaults, semantic projection DTOs, typed request or response row helpers, and Arrow batch validation for the mixed-graph structural plugin lane.
- `xiuxian-julia-core` also owns stable two-node pair projection helpers for that lane, including pair candidate id normalization, pair candidate subgraph projection, and pair-to-request-row builders.
- `xiuxian-julia-core` also owns simple keyword-or-tag query-context builders and binary keyword-or-tag rerank-signal builders for that lane, so host consumers do not manually create anchor DTOs or convert boolean matches into staged plane scores.
- `xiuxian-julia-core` also owns the next convenience layer above those helpers: combined keyword-or-tag pair-rerank request-row builders that compose query-context, rerank-signal, and pair-row projection in one plugin-owned call.
- `xiuxian-julia-core` also owns shared-tag overlap discovery for that lane, including normalized shared-tag anchor extraction and a tag-overlap-aware combined pair-rerank helper.
- `xiuxian-julia-core` also owns the metadata-aware convenience layer above that seam, including node-metadata input bundles and a metadata-aware overlap helper that keeps host consumers from passing ad hoc tag vectors into request projection.
- `xiuxian-julia-core` also owns the metadata-aware batch-assembly layer above that seam, including scored metadata-aware rerank input bundles and a batch helper that composes metadata projection and Arrow request materialization inside the plugin crate.
- `xiuxian-julia-core` also owns the higher-level candidate-input layer above that seam, including single-bundle keyword-overlap request inputs and a batch helper that composes query, metadata, pair, and score staging inside the plugin crate.
- `xiuxian-julia-core` also owns the shared-query and candidate-bundle layer above that seam, including one shared keyword-overlap query bundle, one plugin-owned per-pair candidate bundle, and a batch helper that derives the higher-level request inputs inside the plugin crate.
- `xiuxian-julia-core` also owns the raw-to-candidate staging helper above that seam, so host callers can hand over one pair-input DTO plus raw tag vectors and scores without manually constructing the node-metadata or candidate-bundle DTO layers first.
- `xiuxian-julia-core` also owns the raw-to-query staging helper above that seam, so host callers can hand over raw query identity, layer bounds, keyword anchors, and edge constraints without manually constructing the shared-query DTO layer first.
- `xiuxian-julia-core` also owns the raw-to-pair staging helper above that seam, so host callers can hand over raw pair ids and edge kinds without manually constructing the pair-input DTO layer first.
- `xiuxian-julia-core` also owns the raw pair-metadata-to-candidate staging helper above that seam, so host callers can hand over raw pair ids, edge kinds, left or right tags, and scores without manually composing the metadata-bundle helper and the candidate-bundle helper in sequence.
- `xiuxian-julia-core` also owns the raw-candidate collection batch or fetch seam above that layer, so host callers can hand over one shared query plus raw candidate bundles without manually normalizing each candidate before request-batch or repository-fetch dispatch.
- `xiuxian-julia-core` also now owns one generic explicit-edge topology seam above the pair helpers for structural rerank, so non-pair candidate graphs can be staged and fetched without pair normalization.
- `xiuxian-julia-core` also now owns one raw connected-pair staging seam above the scored pair-collection helper, so host callers can hand over connected pair ids plus semantic scores without first normalizing them into scored pair DTOs.
- `xiuxian-julia-core` also now owns the Julia capability-manifest Arrow seam, including route constants, typed manifest request or response rows, manifest transport option parsing, repository-scoped fetch helpers, manifest-to-binding decoding, and plugin-owned preflight validation against the live Julia capability-manifest route.
...
That block is interpreted in `xiuxian-julia-core` rather than in
...
`xiuxian-julia-core` now falls back to the live `/plugin/capabilities`
...
That block is interpreted in `xiuxian-julia-core` and decoded into manifest
...
`xiuxian-julia-core` so runtime provider selectors stay stable.

The same ownership rule now applies to the typed Rust exchange helpers for
these structural routes:

- semantic projection DTOs live in `xiuxian-julia-core`
- request-row structs and Arrow batch builders live in `xiuxian-julia-core`
- response-row structs and Arrow batch decoders live in `xiuxian-julia-core`
- repository-configured fetch helpers also live in `xiuxian-julia-core`
...
`xiuxian-julia-core`, so the host only forwards raw left or right tag
...
`cargo test -p xiuxian-julia-core graph_structural_exchange --lib` remains
...
- `xiuxian-julia-core` owns the Julia-specific semantic contract and
...
- `direnv exec . cargo test -p xiuxian-julia-core transport --lib`
- `direnv exec . cargo test -p xiuxian-julia-core --lib rerank_exchange`
- `direnv exec . cargo test -p xiuxian-julia-core graph_structural_exchange --lib`
- `direnv exec . cargo test -p xiuxian-julia-core graph_structural_projection --lib`
- `direnv exec . cargo test -p xiuxian-julia-core process_julia_flight_batches_against_real_wendaoarrow_service --lib`
- `direnv exec . cargo test -p xiuxian-julia-core real_wendaoarrow_metadata_example_roundtrip_decodes_trace_id_column --lib`
- `direnv exec . cargo test -p xiuxian-julia-core fetch_graph_structural_keyword_overlap_pair_rerank_rows_for_repository_from_raw_candidates_against_real_wendaosearch_demo_service --lib`
- `direnv exec . cargo check -p xiuxian-julia-core --lib`
