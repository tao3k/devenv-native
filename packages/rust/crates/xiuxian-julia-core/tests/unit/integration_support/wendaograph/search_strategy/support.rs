...
use xiuxian_wendao_runtime::transport::{
    ...
    REPO_SEARCH_SCORE_COLUMN, REPO_SEARCH_TITLE_COLUMN, WENDAO_GRAPH_NODE_ID_HEADER,
};
...
struct SearchStrategyFlowFakeFlightService {
    ...
    graph_neighbors_allowed_node_ids: Option<Arc<HashSet<String>>>,
}
...
impl FlightService for SearchStrategyFlowFakeFlightService {
    ...
    async fn get_flight_info(
        ...
        if route == GRAPH_NEIGHBORS_ROUTE
            && let Some(allowed_node_ids) = &self.graph_neighbors_allowed_node_ids
        {
            let node_id = request
                .metadata()
...
async fn spawn_fake_search_strategy_flow_flight_service_with_graph_node_allowlist(
    scenario: SearchStrategyFlowFakeFlightScenario,
    ...
    let allowed_node_ids = allowed_node_ids
        ...
        .collect::<HashSet<_>>();
    spawn_fake_search_strategy_flow_flight_service_with_batches_and_graph_node_allowlist(
        HashMap::from([
            (REPO_SEARCH_ROUTE.to_owned(), repo_search_batch(scenario)),
            (
                ANALYSIS_REPO_PROJECTED_PAGE_INDEX_TREE_ROUTE.to_owned(),
...
async fn spawn_fake_search_strategy_flow_flight_service_with_batches_and_graph_node_allowlist(
    ...
    let service = SearchStrategyFlowFakeFlightService {
        ...
        graph_neighbors_allowed_node_ids: graph_neighbors_allowed_node_ids.map(Arc::new),
    };
