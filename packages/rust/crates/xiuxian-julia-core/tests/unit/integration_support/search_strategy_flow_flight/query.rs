...
fn candidate_discovery_queries_prioritize_route_scoped_attempts() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            (
                ...
                "packages/rust/crates/xiuxian-julia-core/tests/unit/integration_support/wendaograph/search_strategy"
            ),
        ],
    ...
    assert!(attempts.iter().any(|attempt| {
        attempt.query == "SearchStrategyFlow"
            && attempt.path_prefix == "packages/rust/crates/xiuxian-julia-core/docs"
    }));
    assert!(attempts.iter().any(|attempt| {
...
fn candidate_discovery_queries_start_with_full_intent_for_corpus_recall() {
    ...
    assert!(
        attempts.iter().position(|attempt| {
            attempt.query == "SearchStrategyFlow"
                && attempt.path_prefix == "packages/rust/crates/xiuxian-julia-core/README.md"
        }) > Some(0),
