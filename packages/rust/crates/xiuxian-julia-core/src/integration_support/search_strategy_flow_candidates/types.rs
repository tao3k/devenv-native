...
impl SearchStrategyFlowCandidateInputBatch {
    ...
    pub(crate) fn candidate_input_arrow_snapshot(&self) -> String {
        ...
        for batch in reader {
            let batch = must(batch, "candidate Arrow IPC batch should decode");
            append_candidate_arrow_snapshot_rows(&batch, &mut rows);
        }
...
#[cfg(test)]
fn append_candidate_arrow_snapshot_rows(batch: &RecordBatch, rows: &mut Vec<String>) {
...
#[cfg(test)]
fn int_column<'a>(batch: &'a RecordBatch, name: &str) -> &'a Int64Array {
    batch
        .column_by_name(name)
        .unwrap_or_else(|| panic!("candidate Arrow column `{name}` should exist"))
        .as_any()
