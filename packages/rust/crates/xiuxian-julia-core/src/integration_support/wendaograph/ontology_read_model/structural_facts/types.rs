...
pub(super) struct StructuralProjectionStateRow {
    ...
    pub(super) source_document_count: i64,
    pub(super) source_anchor_count: i64,
}
...
pub(super) fn required_value<'a>(
    ...
    row.get(column_name).map(String::as_str).ok_or_else(|| {
        ...
        format!("structural facts `{table_name}` row {row_label} is missing `{column_name}`")
    })
}
