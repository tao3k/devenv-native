...
/// Named request for running `SearchStrategyFlow` with optional side-table
/// inputs.
#[derive(Debug, Clone)]
pub struct SearchStrategyFlowSideTableExecutionRequest {
    intent: String,
    search_root: PathBuf,
    config: Option<SearchStrategyFlowFlightMaterializationConfig>,
...
impl SearchStrategyFlowSideTableExecutionRequest {
    /// Build a side-table execution request with no optional tables.
    #[must_use]
    pub fn new(intent: impl Into<String>, search_root: impl Into<PathBuf>) -> Self {
        Self {
            intent: intent.into(),
            search_root: search_root.into(),
            config: None,
            query_understanding_arrow_ipc_path: String::new(),
            branch_judgements_arrow_ipc_path: String::new(),
        }
    }

    /// Attach Flight materialization config.
    #[must_use]
    pub fn with_flight_materialization_config(
        mut self,
        config: Option<SearchStrategyFlowFlightMaterializationConfig>,
    ) -> Self {
        self.config = config;
    ...
    /// Attach an optional query-understanding Arrow IPC side table path.
    #[must_use]
    pub fn with_query_understanding_arrow_ipc_path(mut self, path: impl Into<String>) -> Self {
        self.query_understanding_arrow_ipc_path = path.into();
        self
    }

    /// Attach an optional branch-judgement Arrow IPC side table path.
    #[must_use]
    pub fn with_branch_judgements_arrow_ipc_path(mut self, path: impl Into<String>) -> Self {
        self.branch_judgements_arrow_ipc_path = path.into();
        self
    }
}
...
pub async fn run_wendaograph_search_strategy_flow_json_with_flight_materialization_and_branch_judgements(
    ...
    run_wendaograph_search_strategy_flow_json_with_flight_materialization_and_side_tables(
        SearchStrategyFlowSideTableExecutionRequest::new(intent, search_root)
...
pub async fn run_wendaograph_search_strategy_flow_json_with_flight_materialization_and_side_tables(
    request: SearchStrategyFlowSideTableExecutionRequest,
) -> Result<String, String> {
    let SearchStrategyFlowSideTableExecutionRequest {
        intent,
        search_root,
        config,
    ...
    let trace = match config.as_ref() {
        Some(config) => {
            let candidate_batch =
                search_strategy_flow_candidate_input_batch_from_repo_search(&intent, config)
                    .await?;
            ...
            run_wendaograph_search_strategy_flow_raw_json_with_candidate_batch(
                ...
                search_root.as_path(),
                candidate_batch,
                query_understanding_arrow_ipc_path.as_str(),
                branch_judgements_arrow_ipc_path.as_str(),
