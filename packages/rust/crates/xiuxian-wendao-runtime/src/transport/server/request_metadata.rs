...
use crate::transport::query_contract::{
    ...
    WENDAO_PDF_EXTRACT_FORMULAS_HEADER, WENDAO_PDF_EXTRACT_IMAGES_HEADER,
    WENDAO_PDF_EXTRACT_OUTPUT_DIR_HEADER, WENDAO_PDF_EXTRACT_SOURCE_PATH_HEADER,
    WENDAO_PDF_EXTRACT_TABLES_HEADER, WENDAO_REFINE_DOC_ENTITY_ID_HEADER,
    ...
    validate_pdf_extract_request, validate_refine_doc_request, validate_repo_doc_coverage_request,
    validate_repo_index_request, validate_repo_index_status_request, validate_repo_overview_request,
...
pub(crate) fn validate_pdf_extract_request_metadata(
    metadata: &MetadataMap,
) -> Result<(String, String, bool, bool, bool), Status> {
    let source_path = metadata
        .get(WENDAO_PDF_EXTRACT_SOURCE_PATH_HEADER)
        .and_then(|value| value.to_str().ok())
        .unwrap_or_default()
        .to_string();
    validate_pdf_extract_request(source_path.as_str()).map_err(Status::invalid_argument)?;
    let output_dir = metadata
        .get(WENDAO_PDF_EXTRACT_OUTPUT_DIR_HEADER)
        .and_then(|value| value.to_str().ok())
        .unwrap_or_default()
        .to_string();
    let extract_images = metadata
        .get(WENDAO_PDF_EXTRACT_IMAGES_HEADER)
        .and_then(|value| value.to_str().ok())
        .map(|value| value == "true" || value == "1")
        .unwrap_or(true);
    let extract_tables = metadata
        .get(WENDAO_PDF_EXTRACT_TABLES_HEADER)
        .and_then(|value| value.to_str().ok())
        .map(|value| value == "true" || value == "1")
        .unwrap_or(true);
    let extract_formulas = metadata
        .get(WENDAO_PDF_EXTRACT_FORMULAS_HEADER)
        .and_then(|value| value.to_str().ok())
        .map(|value| value == "true" || value == "1")
        .unwrap_or(true);
    Ok((source_path, output_dir, extract_images, extract_tables, extract_formulas))
}
