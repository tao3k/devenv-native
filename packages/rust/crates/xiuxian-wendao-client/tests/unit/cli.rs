...
fn parses_get_toc_command() {
    ...
    match command {
        ...
        GetCommand::StructureCatalog(_) => panic!("expected get toc command"),
    }
}

#[test]
fn parses_get_structure_catalog_command() {
    let cli = ClientCli::parse_from(["wendao", "get", "structure-catalog"]);
    let ClientCommand::Get { command } = cli.command else {
        panic!("expected get command");
    };
    match command {
        GetCommand::StructureCatalog(args) => assert_eq!(args.target, PathBuf::from(".")),
        GetCommand::Toc(_) => panic!("expected get structure-catalog command"),
    }
}
