...
use super::{
    ...
    effective_section_level, render,
};
use crate::{ClientContext, CommandOutcome};

#[cfg(test)]
use super::render::{render_page_index_markdown, render_toc_markdown};
...
pub(crate) struct CanonicalScopeTarget {
    path: PathBuf,
...
fn handle_toc(args: &GetScopeArgs, context: &ClientContext) -> Result<CommandOutcome> {
    ...
    render::emit_toc_output(&result, context.output())?;
...
fn handle_page_index(args: &GetScopeArgs, context: &ClientContext) -> Result<CommandOutcome> {
    ...
    render::emit_page_index_output(&result, context.output())?;
    Ok(CommandOutcome::success())
}

pub(crate) fn canonical_scope_target(cwd: &Path, target: &Path) -> Result<CanonicalScopeTarget> {
...
pub(crate) fn build_local_page_index_trees_with_ignore(
    scope: &CanonicalScopeTarget,
    client_root: &Path,
    ignore_dir_names: &[String],
) -> Result<DocsPageIndexTreesResult> {
...
pub(crate) fn default_ignore_dir_names() -> Vec<String> {
