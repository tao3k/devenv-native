...
pub(crate) const CHECKPOINT_TIMESTAMP_COLUMN: &str = "checkpoint_timestamp";
pub(crate) const CHECKPOINT_PARENT_ID_COLUMN: &str = "checkpoint_parent_id";
pub(crate) const CHECKPOINT_STEP_COLUMN: &str = "checkpoint_step";
...
impl CheckpointStore {
    ...
    pub async fn cleanup_orphan_checkpoints(
                    ...
                    for i in 0..batch.num_rows() {
                        ...
                        if !metas.is_null(i) {
                            ...
                            if let Ok(meta) = serde_json::from_str::<serde_json::Value>(meta_str) {
                                if let Some(parent) =
                                    meta.get("parent_id").and_then(serde_json::Value::as_str)
                                {
        ...
        if orphan_count > 0 {
            log::info!(
                "Found {orphan_count} orphan checkpoints in {table_name} (dry_run={dry_run})"
            );
    ...
    /// This method is optimized for fast timeline rendering in Python and Rust.
    ///
    /// # Arguments
    /// * `table_name` - Name of the checkpoint table
    ...
    pub async fn get_timeline_records(
        ...
        thread_id: &str,
        limit: usize,
    ) -> Result<Vec<TimelineRecord>, VectorStoreError> {
        let table_path = self.table_path(table_name);
        if !table_path.exists() {
            return Ok(Vec::new());
        }
        ...
        let mut checkpoints: Vec<(f64, TimelineRecord)> = Vec::new();
        ...
        let timeline: Vec<TimelineRecord> = checkpoints
            .into_iter()
            .enumerate()
            .map(|(step, (_, record))| {
                let mut record = record;
                record.step = i32::try_from(step).unwrap_or(i32::MAX);
                record
            })
            .collect();
