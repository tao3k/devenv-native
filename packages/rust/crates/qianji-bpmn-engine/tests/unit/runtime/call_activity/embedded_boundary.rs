...
async fn runtime_embedded_subprocess_interrupting_timer_boundary_routes_parent_path() {
    ...
    let mut instance = create_instance(
        ...
        BpmnInstanceInit::new(
            "wf_embedded_subprocess_timer_boundary",
            json!({ "amount": 7 }),
            10,
        ),
    )
    ...
    .must("timer outcome should cancel the embedded subprocess and route the parent boundary");
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_embedded_subprocess_success_clears_interrupting_timer_boundary_wait() {
    let package = Arc::new(parsed_fixture_package(
        "embedded-subprocess-timer-boundary.bpmn",
    ));
    let mut instance = create_instance(
        ...
        "main_process",
        BpmnInstanceInit::new(
            "wf_embedded_subprocess_timer_boundary_success",
            json!({ "amount": 7 }),
            10,
        ),
    )
    ...
    let completion = advance_instance(package.as_ref(), &mut instance, &host)
        .await
        .must(
            "embedded subprocess should complete normally and clear the parent timer boundary wait",
        );
    assert_eq!(completion, BpmnAdvanceOutcome::Completed);
    assert_eq!(instance.lifecycle, InstanceLifecycle::Completed);
    assert_eq!(instance.process.process_id.as_ref(), "main_process");
