...
async fn runtime_embedded_subprocess_escalation_routes_specific_and_catch_all_boundaries() {
    ...
    let mut instance = create_instance(
        ...
        BpmnInstanceInit::new(
            "wf_embedded_subprocess_escalation",
            json!({ "amount": 7 }),
            10,
        ),
    )
    ...
    let completion = advance_instance(package.as_ref(), &mut instance, &host)
        .await
        .must("embedded subprocess escalation end should route through matching parent boundaries");
    ...
    assert_eq!(
        instance.node_states
            [node_index(&package, "main_process", "review_escalation_specific") as usize]
            .status,
        qianji_bpmn_engine::NodeRuntimeStatus::Completed
    );
    assert_eq!(
        instance.node_states
            [node_index(&package, "main_process", "review_escalation_catch_all") as usize]
            .status,
        qianji_bpmn_engine::NodeRuntimeStatus::Completed
    );
    assert_eq!(
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_embedded_subprocess_intermediate_escalation_routes_parent_boundaries() {
    let package = Arc::new(parsed_fixture_package(
    ...
    let mut instance = create_instance(
        ...
        "main_process",
        BpmnInstanceInit::new(
            "wf_embedded_subprocess_intermediate_escalation",
            json!({ "amount": 7 }),
            10,
        ),
    )
    ...
    let completion = advance_instance(package.as_ref(), &mut instance, &host)
        .await
        .must("embedded subprocess intermediate escalation should route parent boundaries");
    assert_eq!(completion, BpmnAdvanceOutcome::Completed);
    assert_eq!(instance.lifecycle, InstanceLifecycle::Completed);
    assert_eq!(instance.process.process_id.as_ref(), "main_process");
    assert!(instance.call_stack.is_empty());
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_embedded_subprocess_success_cancels_escalation_boundaries() {
    let package = Arc::new(parsed_fixture_package(
        "embedded-subprocess-escalation-boundary.bpmn",
    ));
    let mut instance = create_instance(
        ...
        "main_process",
        BpmnInstanceInit::new(
            "wf_embedded_subprocess_escalation_success",
            json!({ "amount": 7 }),
            10,
        ),
    )
    ...
    let completion = advance_instance(package.as_ref(), &mut instance, &host)
        .await
        .must("embedded subprocess should complete normally and cancel escalation boundaries");
    assert_eq!(completion, BpmnAdvanceOutcome::Completed);
    assert_eq!(instance.lifecycle, InstanceLifecycle::Completed);
    assert_eq!(instance.process.process_id.as_ref(), "main_process");
    assert!(instance.call_stack.is_empty());
