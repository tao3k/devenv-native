...
| BPMN DI                   | metadata-only      | Diagram, plane, shape, edge, bounds, waypoint, label, and font metadata is preserved; DI namespaces, topology, anchor presence/kind, links, ids, and minimum layout payloads are audited. |
...
`BPMNShape`, and at least two direct `di:waypoint` entries on `BPMNEdge`.
Coordinate values, geometry quality, diagram rendering, and runtime routing
from layout coordinates remain out of scope.
