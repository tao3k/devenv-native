...
pub(crate) fn evaluate_dmn_package_binding_sync(
    ...
    let output_decisions =
        service::resolve_decision_service_output_decisions(package, decision_service)?;
    service::validate_decision_service_exposure_contract(package, decision_service)?;
    ...
    service::evaluate_dmn_package_decision_service_outputs_sync(
        package,
        decision_service,
        &output_decisions,
        variables,
...
fn evaluate_dmn_package_decision_with_stack(
    ...
    let result = (|| {
        ...
        if let Some(invocation) = decision.invocation.as_ref() {
            return service::evaluate_dmn_invocation(package, decision, invocation, &variables);
        }
