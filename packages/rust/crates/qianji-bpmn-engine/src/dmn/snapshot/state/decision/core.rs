use super::{
    BytesStart, DmnDecisionSnapshot, DmnSourceFile, Reader, Result, TempFunctionDefinitionSnapshot,
    TempInvocationSnapshot, TempRequirementReferenceSnapshot, attribute_value, required_attribute,
};

pub(in crate::dmn::snapshot::state) struct TempDecisionSnapshot {
    decision_id: String,
    name: Option<String>,
...
impl TempDecisionSnapshot {
    ...
    pub(in crate::dmn::snapshot::state) fn track_construct(
        &mut self,
        tag: &str,
    ...
    pub(in crate::dmn::snapshot::state) fn push_invocation(
        &mut self,
        invocation: TempInvocationSnapshot,
    ) {
        self.invocations.push(invocation);
    }

    pub(in crate::dmn::snapshot::state) fn push_function_definition(
        &mut self,
    ...
    pub(in crate::dmn::snapshot::state) fn push_requirement_reference(
        &mut self,
