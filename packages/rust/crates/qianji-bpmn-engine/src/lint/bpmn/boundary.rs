...
fn non_interrupting_boundary_requires_supported_task_repeat_owner_issue(
    ...
    LintIssue::new(
        ...
        format!(
            "Repair process '{process_id}' so boundary event '{node_id}' uses one non-interrupting timer, message, or signal boundary only on one non-repeating serviceTask, scriptTask, userTask, manualTask, or businessRuleTask, or on one bounded standard-loop, sequential multi-instance, or parallel multi-instance owner of those same task kinds, with exactly one timer, message, or signal definition. Preserve workflow intent, but do not keep `cancelActivity=\"false\"` on a task owner whose repeat metadata falls outside that bounded subset."
        ),
        json!({
            "process_id": process_id,
            "node_id": node_id,
...
fn generic_boundary_configuration_issue(
    ...
    LintIssue::new(
        ...
        "The current engine supports one interrupting timer, message, or signal boundary on one host-blocking task, one interrupting timer, message, or signal boundary on one bounded embedded subprocess owner either alone or paired with one or more interrupting error boundaries on that same owner, one interrupting timer, message, or signal boundary on one bounded same-package call activity owner either alone or paired with one or more interrupting error boundaries on that same owner, one interrupting timer, message, or signal boundary on one bounded transaction shell either on its own, paired with one interrupting cancel boundary, paired with one or more interrupting error boundaries on that same owner, or paired with one interrupting cancel boundary plus one or more interrupting error boundaries on that same owner, one non-interrupting timer, message, or signal boundary on one non-repeating or bounded standard-loop, sequential multi-instance, or parallel multi-instance host-blocking task, one interrupting cancel boundary on one bounded transaction shell, and one or more interrupting error boundaries on one bounded transaction shell, one bounded embedded subprocess shell, or one bounded same-package call activity owner.",
        vec![
