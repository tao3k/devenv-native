...
fn compensation_requires_transaction_shell_issue(
    ...
    LintIssue::new(
        ...
        "The current engine only executes bounded compensation inside one nested `<bpmn:transaction>` body: either through explicit boundary-to-handler replay during transaction cancel, one synchronous targeted throw-compensation end event, or one synchronous targeted throw-compensation intermediate event that references an already compensable activity in that same transaction shell.",
        vec![
...
fn default_compensation_end_event_issue(
    ...
    LintIssue::new(
        ...
        "The current engine does not execute default compensation from throwing end events. It only executes three bounded compensation shapes inside one transaction shell: explicit boundary-to-handler replay during transaction cancel, one synchronous targeted throw-compensation end event with explicit `activityRef`, and one synchronous targeted throw-compensation intermediate event with explicit `activityRef`.",
        vec![
...
fn default_compensation_intermediate_event_issue(
    ...
    LintIssue::new(
        ...
        "The current engine does not execute default compensation from throwing intermediate events. It only executes three bounded compensation shapes inside one transaction shell: explicit boundary-to-handler replay during transaction cancel, one synchronous targeted throw-compensation end event with explicit `activityRef`, and one synchronous targeted throw-compensation intermediate event with explicit `activityRef`.",
        vec![
            "If transaction rollback is really required, remodel the flow with one bounded transaction cancel path plus explicit compensation boundary-to-handler bindings on the completed activities.".to_string(),
            "If the flow only needs ordinary progression, remove `<compensateEventDefinition>` and keep normal sequence-flow routing to the next supported node.".to_string(),
...
fn generic_compensation_configuration_issue(
    ...
    LintIssue::new(
        ...
        vec![
            ...
            "Use throw-compensation end or intermediate events only when they stay synchronous, target one explicit `activityRef`, and point to one activity that already owns a valid compensation boundary-to-handler binding.".to_string(),
            "Do not place loops, multi-instance characteristics, compensation event subprocesses, default compensation, or asynchronous throw-compensation forms on this bounded compensation path.".to_string(),
        ],
        format!(
            "Repair compensation node '{node_id}' in process '{process_id}' so it fits the bounded compensation slice: keep it inside one `<bpmn:transaction>` shell, use explicit boundary-to-handler bindings for compensable activities, and only use throw-compensation end or intermediate events when they stay synchronous and target one explicit `activityRef` that already owns a valid compensation handler binding. Remove unsupported configuration '{detail}'."
        ),
        json!({
            "process_id": process_id,
            "node_id": node_id,
