//! Root-doc secondary-owner checks for LLM-friendly Rust module layouts.
...
/// Result of evaluating whether the secondary seam named in the root doc is canonical.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum RootDocSecondaryOwnerCheck {
    /// The file is not a root-seam candidate or the rule does not apply.
    NotApplicable,
    /// The root doc uses a canonical-looking secondary seam.
    CanonicalSecondary,
    /// The root doc points its secondary seam at a helper/detail bucket.
    SuspiciousSecondary(RootDocSecondaryOwnerMetrics),
}

/// Metrics for one suspicious secondary seam named in the root doc.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct RootDocSecondaryOwnerMetrics {
    ...
    /// Helper/detail bucket named as the secondary seam.
    pub(crate) secondary_module: String,
    /// 1-based line number of the first root doc attribute.
    pub(crate) doc_line_number: usize,
}

/// Check whether the secondary seam named in an internal root doc is canonical.
#[must_use]
pub(crate) fn check_root_doc_secondary_owner(
    path: &Path,
    text: &str,
    file_texts: &BTreeMap<PathBuf, String>,
) -> RootDocSecondaryOwnerCheck {
    if !is_root_seam_candidate(path) {
        return RootDocSecondaryOwnerCheck::NotApplicable;
    }
    ...
    if matches!(parent_visibility, ParentModuleVisibility::Public) {
        return RootDocSecondaryOwnerCheck::CanonicalSecondary;
    }
    ...
    if named_modules.len() != 2 {
        return RootDocSecondaryOwnerCheck::CanonicalSecondary;
    }

    let Some(secondary_module) = named_modules
        .keys()
        .find(|module_name| *module_name != &visible_owner)
        .cloned()
    else {
        return RootDocSecondaryOwnerCheck::CanonicalSecondary;
    };

    if !SUSPICIOUS_OWNER_MODULES.contains(&secondary_module.as_str()) {
        return RootDocSecondaryOwnerCheck::CanonicalSecondary;
    }

    RootDocSecondaryOwnerCheck::SuspiciousSecondary(RootDocSecondaryOwnerMetrics {
