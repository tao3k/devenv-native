...
use xiuxian_wendao::analyzers::{
    ImportKind, ImportRecord, RepoSymbolKind, RepositoryAnalysisOutput,
};
...
pub(crate) fn build_code_ast_analysis_response(
    ...
    for relation in &analysis.relations {
        let kind = match relation.kind {
            xiuxian_wendao::analyzers::RelationKind::Contains => {
                contains_edge_count += 1;
                CodeAstEdgeKind::Contains
            }
            xiuxian_wendao::analyzers::RelationKind::Calls => {
                interaction_edge_count += 1;
                CodeAstEdgeKind::Calls
            }
            xiuxian_wendao::analyzers::RelationKind::Uses => {
                interaction_edge_count += 1;
                uses_edge_count += 1;
                CodeAstEdgeKind::Uses
            }
            xiuxian_wendao::analyzers::RelationKind::Imports => {
                interaction_edge_count += 1;
