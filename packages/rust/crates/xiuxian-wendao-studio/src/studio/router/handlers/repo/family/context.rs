...
use crate::studio::router::handlers::repo::projected_service::family::{
    run_repo_projected_page_family_cluster, run_repo_projected_page_family_context,
    run_repo_projected_page_family_search,
};
...
) -> Result<Json<xiuxian_wendao::analyzers::RepoProjectedPageFamilyContextResult>, StudioApiError> {
...
pub async fn projected_page_family_search(
    ...
    State(state): State<Arc<GatewayState>>,
) -> Result<Json<xiuxian_wendao::analyzers::RepoProjectedPageFamilySearchResult>, StudioApiError> {
    let repo_id = required_registered_repo_id(state.studio.as_ref(), query.repo.as_deref())?;
...
pub async fn projected_page_family_cluster(
    ...
    State(state): State<Arc<GatewayState>>,
) -> Result<Json<xiuxian_wendao::analyzers::RepoProjectedPageFamilyClusterResult>, StudioApiError> {
    let repo_id = required_registered_repo_id(state.studio.as_ref(), query.repo.as_deref())?;
    let page_id = required_page_id(query.page_id.as_deref())?;
