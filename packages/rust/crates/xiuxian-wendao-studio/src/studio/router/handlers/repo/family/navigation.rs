//! Owns the Studio repo family navigation surface.
...
) -> Result<Json<xiuxian_wendao::analyzers::RepoProjectedPageNavigationResult>, StudioApiError> {
...
pub async fn projected_page_navigation_search(
    ...
    State(state): State<Arc<GatewayState>>,
) -> Result<Json<xiuxian_wendao::analyzers::RepoProjectedPageNavigationSearchResult>, StudioApiError>
{
    let repo_id = required_registered_repo_id(state.studio.as_ref(), query.repo.as_deref())?;
