...
use crate::studio::router::handlers::docs::service::projection::navigation::{
...
use xiuxian_wendao::analyzers::{DocsNavigationQuery, DocsNavigationSearchQuery};
...
) -> Result<Json<xiuxian_wendao::analyzers::DocsNavigationResult>, StudioApiError> {
...
pub async fn navigation_search(
    ...
    State(state): State<Arc<GatewayState>>,
) -> Result<Json<xiuxian_wendao::analyzers::DocsNavigationSearchResult>, StudioApiError> {
    let repo_id = required_registered_repo_id(state.studio.as_ref(), query.repo.as_deref())?;
