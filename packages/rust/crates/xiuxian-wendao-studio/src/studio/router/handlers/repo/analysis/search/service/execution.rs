            ...
            move || async move {
                ...
                with_repo_cached_analysis_bundle(
                    ...
                    move |cached| {
                        ...
                        if let Some(result) =
                            xiuxian_wendao::analyzers::load_cached_repository_search_result(
                                &cache_key,
                            )?
                        {
                            return Ok(result);
                        ...
                        xiuxian_wendao::analyzers::store_cached_repository_search_result(
                            &cache_key, &result,
                        )?;
                        Ok(result)
                    },
...
where
    ...
    BuildFn: FnOnce(
        ...
        &xiuxian_wendao::analyzers::RepositorySearchArtifacts,
    ) -> T,
{
    let cache_key = repository_search_key(&cached.cache_key, scope, query, limit, fuzzy_options);
    if let Some(result) =
        xiuxian_wendao::analyzers::load_cached_repository_search_result(&cache_key)?
    {
        return Ok(result);
    }
    ...
    xiuxian_wendao::analyzers::store_cached_repository_search_result(&cache_key, &result)?;
    Ok(result)
}
