...
use super::{
    BTreeMap, BTreeSet, DOCUMENT_EXTRACT_OCR_SHARD_CACHE_ROOT_ENV,
    DOCUMENT_EXTRACT_PDF_RENDER_REGIONS_ENV, Digest, HybridPdfOcr2ScaffoldMode,
    OCR_PENDING_RESOURCE_ARROW_NAME, OCR_SHARD_INPUT_ARROW_NAME, OCR_SHARD_MANIFEST_ARROW_NAME,
    OCR2_REGION_RENDER_CACHE_DIR_NAME, OCR2_REGION_SCAFFOLD_FILE_NAME, Path, PathBuf,
    PdfOcrShardInput, PdfPageRegionRenderRequest, PdfPageRenderProfile, PdfPageRenderShardReport,
    PdfRenderRoutingDecision, PdfRenderStatus, PdfSourcePageProfile, Sha256, Value,
    automatic_ocr2_recovery_region_requests_for_source_with_lookup, decode_ocr_shard_input_batches,
    hybrid_page_ocr_input_arrow_path, hybrid_page_ocr_region_requests_for_source_with_lookup,
    hybrid_page_ocr_render_profile_with_lookup, hybrid_page_ocr2_scaffold_mode_with_lookup,
    is_hosted_vlm_direct_profile, json, merge_hosted_vlm_recovery_region_inputs, read_arrow_file,
...
use super::{Instant, Ocr2RegionMaterialization};
#[cfg(not(feature = "document-extract-pdf-render"))]
use super::{PdfOcrShardInput, PdfPageRenderShardReport};

pub(super) async fn materialize_ocr2_recovery_page_images(
...
pub(super) async fn materialize_ocr2_recovery_region_images(
    render_report: &PdfPageRenderShardReport,
    inputs: Vec<PdfOcrShardInput>,
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn ocr2_recovery_region_requests_for_inputs(
    source_path: &Path,
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn ocr2_region_render_cache_dir(
    source: &Path,
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn ocr2_region_render_cache_root() -> PathBuf {
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn ocr2_region_render_cache_key(
    source: &Path,
    profile: &PdfPageRenderProfile,
    regions: &[PdfPageRegionRenderRequest],
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn cached_ocr2_region_render_report(
    source: &Path,
    output_dir: &Path,
    page_count: u32,
    profile: &PdfPageRenderProfile,
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn write_ocr2_region_scaffold_sidecar_with_lookup(
    source: &Path,
    output_dir: &Path,
    inputs: &[PdfOcrShardInput],
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn ocr2_region_scaffold_payload(
    source: &Path,
    inputs: &[PdfOcrShardInput],
    explicit_regions: bool,
    lookup: &dyn Fn(&str) -> Option<String>,
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn ocr2_region_scaffold_kind(
...
#[cfg(any(feature = "document-extract-pdf-render", test))]
pub(super) fn source_page_profile_json(profile: &PdfSourcePageProfile) -> Value {
    json!({
