...
async fn plugin_artifact_handler_returns_resolved_artifact() {
    ...
    let response = crate::studio::router::handlers::capabilities::get_plugin_artifact(
        State(state),
        Path(artifact_path.clone()),
        Query(crate::studio::router::handlers::capabilities::PluginArtifactQuery { format: None }),
    )
    .await
...
async fn plugin_artifact_handler_returns_toml_when_requested() {
    ...
    let response = crate::studio::router::handlers::capabilities::get_plugin_artifact(
        ...
        Path(artifact_path),
        Query(
            crate::studio::router::handlers::capabilities::PluginArtifactQuery {
                format: Some(
                    xiuxian_wendao::zhenfa_router::native::WendaoPluginArtifactOutputFormat::Toml,
                ),
            },
        ),
    )
