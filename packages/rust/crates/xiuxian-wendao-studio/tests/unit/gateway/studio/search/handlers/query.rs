...
use crate::studio::search::handlers::test_prelude::SearchQuery;
use xiuxian_wendao::analyzers::{RepoBacklinkItem, RepoSymbolKind, SymbolSearchHit};
use xiuxian_wendao::parsers::search::repo_code_query::parse_repo_code_search_query;
use xiuxian_wendao::search::repo_search::{collect_repo_search_targets, repo_search_parallelism};
use xiuxian_wendao::search::{RepoSearchAvailability, RepoSearchPublicationState};
...
fn infer_repo_hint_from_repositories_matches_url_basename_alias() {
    ...
    let repositories = vec![xiuxian_wendao::analyzers::RegisteredRepository {
        id: "lancd".to_string(),
        path: None,
        url: Some("https://github.com/lance-format/lance".to_string()),
        git_ref: None,
        refresh: xiuxian_wendao::analyzers::RepositoryRefreshPolicy::Fetch,
        plugins: vec![],
    }];
...
fn query_uses_redundant_repo_seed_for_structural_repo_alias_query() {
    ...
    let repository = xiuxian_wendao::analyzers::RegisteredRepository {
        id: "lancd".to_string(),
        path: None,
        url: Some("https://github.com/lance-format/lance".to_string()),
        git_ref: None,
...
fn repo_search_parallelism_reuses_search_plane_read_budget() {
    let studio = crate::studio::search::handlers::tests::test_studio_state();
    let expected_budget = studio.search_plane.repo_search_parallelism(usize::MAX);
...
fn symbol_search_hit_to_search_hit_preserves_backend_metadata() {
    let hit = symbol_search_hit_to_search_hit(
        "sciml",
        SymbolSearchHit {
            symbol: xiuxian_wendao::analyzers::SymbolRecord {
