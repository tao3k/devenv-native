use super::{
    EpistemeCommand, EpistemeSourceContractCommand, EpistemeWriteStructuralIdfReasoningPacketArgs,
};

#[test]
fn episteme_source_contract_write_structural_idf_reasoning_packet_args_capture_input_and_filters() {
    let args = EpistemeWriteStructuralIdfReasoningPacketArgs {
        ...
        structural_idf_run_id: "structural_seed".to_string(),
    ...
    assert_eq!(args.structural_idf_run_id, "structural_seed");
...
fn episteme_source_contract_command_debug_names_reasoning_packet_variant() {
    let command = EpistemeCommand::SourceContract {
        command: EpistemeSourceContractCommand::WriteStructuralIdfReasoningPacket(
    ...
    assert!(rendered.contains("WriteStructuralIdfReasoningPacket"));
}
