use super::{
    EpistemeCommand, EpistemeSourceContractCommand, EpistemeWriteStructuralIdfReasoningFillPlanArgs,
};

#[test]
fn episteme_source_contract_write_reasoning_fill_plan_args_capture_input_and_limit() {
    let args = EpistemeWriteStructuralIdfReasoningFillPlanArgs {
...
fn episteme_source_contract_command_debug_names_reasoning_fill_plan_variant() {
    let command = EpistemeCommand::SourceContract {
        command: EpistemeSourceContractCommand::WriteStructuralIdfReasoningFillPlan(
    ...
    assert!(rendered.contains("WriteStructuralIdfReasoningFillPlan"));
}
