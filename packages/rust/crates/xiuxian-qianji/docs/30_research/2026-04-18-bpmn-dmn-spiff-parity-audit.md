...
1. `packages/rust/crates/qianji-bpmn-engine/src/lib.rs` now states that bounded
   ...
   synchronous targeted throw-compensation `endEvent` subset and one
   synchronous targeted throw-compensation `intermediateThrowEvent` subset,
   each with explicit `activityRef` inside that same transaction shell, plus
   ...
   compensation, asynchronous or default throw-compensation forms, more than
   one cancel boundary on one transaction owner, broader transaction error
...
2. `packages/rust/crates/qianji-bpmn-engine/src/parser/import.rs` now accepts
   ...
   plus one or more error boundaries, plus one synchronous targeted
   throw-compensation `endEvent` subset and one synchronous targeted
   throw-compensation `intermediateThrowEvent` subset with explicit
   `activityRef` inside that same transaction shell.
3. `packages/rust/crates/qianji-bpmn-engine/src/runtime/lifecycle.rs` now
   ...
   completion order before the parent cancel boundary fires, plus one
   synchronous targeted throw-compensation `endEvent` path and one
   synchronous targeted throw-compensation `intermediateThrowEvent` path that
   each replay one referenced compensable activity inside that same
   transaction shell, plus bounded standard-loop re-entry and skip semantics,
   plus bounded
   ...
   asynchronous or default throw-compensation forms, more than one cancel
