...
1. the crate still does not execute direct `invocation` decisions, and direct
   `context` execution is limited to the bounded context-entry subset recorded
   in the follow-up above
2. the non-executable DMN snapshot surface can now classify those constructs
   explicitly; a later snapshot-evidence slice also preserves direct
   invocation function-expression and binding placeholders, which closes
   another real adapter/lint gap against the broader versioned DMN documents
   present in the imported `SpiffWorkflow` research lane
...
The earlier direct `invocation` placeholder statement is now more precise.

What changed:

1. direct decision-owned `<invocation>` remains non-executable, so there is no
   claim of invocation-runtime parity
...
3. `qianji lint --dmn` now includes that invocation evidence in
   `dmn.unsupported_invocation_decision`, so LLM repair flows can preserve the
   called function name and binding shape instead of guessing or flattening
   invocation logic
4. called-function resolution, business-knowledge-model execution, binding
   ...
   broader FEEL semantics remain explicitly deferred
