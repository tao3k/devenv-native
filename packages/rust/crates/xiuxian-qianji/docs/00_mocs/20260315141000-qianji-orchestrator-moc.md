...
- [[2026-04-23-qianji-duckdb-workflow-data-store]]: DuckDB-backed local workflow data records and no-server workflow-state snapshots through `xiuxian-db-store`, while `qianji-server` HTTP control remains Valkey-backed.
...
- the shared control-plane markdown renderer path is now deduplicated through a
  Qianji-local embedded-template catalog helper: `show`, `check`,
  Flowhub-root/module fragments, and Flowhub-scenario preview fragments all
  reuse one template-catalog surface instead of each maintaining a separate
...
- `qianji-client flowhub --mode plan --scenario agent-coding init` is now the downstream-facing plan initialization surface provided by `xiuxian-qianji-client`. The older localized run-root materializer remains a library/runtime primitive for Mermaid-enriched Flowhub run experiments, but it is no longer advertised as a top-level `qianji materialize` command. This keeps `qianji` focused on engine/check/control surfaces while `xiuxian-qianji-client` owns reusable downstream Flowhub setup.
