...
- `qianji-client flowhub --mode plan --scenario agent-coding init` is now the downstream-facing plan initialization surface. The older localized run-root materializer remains a library/runtime primitive for Mermaid-enriched Flowhub run experiments, but it is no longer advertised as a top-level `qianji materialize` command. This keeps `qianji` focused on engine/check/control surfaces while `qianji-client` owns reusable downstream Flowhub setup.
