...
async fn run_prepared_bpmn_task_complete_command(
    command: &BpmnTaskCompleteCliCommand,
    control_service: &crate::qianji_cli::bpmn_cli::deps::QianjiBpmnWorkflowControlService,
    task_complete_request: &crate::qianji_cli::bpmn_cli::deps::QianjiBpmnWorkflowTaskCompleteRequest,
    host_context: &crate::qianji_cli::bpmn_cli::types::BpmnCliHostBridgeContext,
) -> Result<BpmnCliOutput, Box<dyn std::error::Error>> {
    match control_service
...
fn trace_observer(
    ...
    &crate::qianji_cli::bpmn_cli::deps::QianjiBpmnSession,
    &[crate::qianji_cli::bpmn_cli::deps::BpmnExecutionTraceEvent],
) {
    move |session, events| {
        if enabled {
