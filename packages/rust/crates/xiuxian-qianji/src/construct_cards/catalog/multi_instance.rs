...
pub(super) const fn card(lint_mappings: &'static [ConstructLintMapping]) -> ConstructCard {
    ConstructCard {
        ...
        requires: &[
            "one serviceTask with native BPMN documentation and IO metadata",
            ...
            "when collection-backed, dataInputAssociation should bind the per-iteration inputDataItem name",
        ...
        forbids: &[
            ...
            "completionCondition expressions outside the bounded native subset",
  ...
  <bpmn:process id="Process_ParallelAgents" isExecutable="true">
    ...
    <bpmn:serviceTask id="Task_RunAgent" name="Run one agent task" implementation="${environment.services.runAgent}">
      <bpmn:documentation>Run the focused agent task in agentTask. Return JSON with agentResult.</bpmn:documentation>
      <bpmn:ioSpecification>
        <bpmn:dataInput id="Task_RunAgent_Input_agentTask" name="agentTask"/>
        <bpmn:dataOutput id="Task_RunAgent_Output_agentResult" name="agentResult"/>
        <bpmn:inputSet><bpmn:dataInputRefs>Task_RunAgent_Input_agentTask</bpmn:dataInputRefs></bpmn:inputSet>
        <bpmn:outputSet><bpmn:dataOutputRefs>Task_RunAgent_Output_agentResult</bpmn:dataOutputRefs></bpmn:outputSet>
      </bpmn:ioSpecification>
      <bpmn:dataInputAssociation><bpmn:sourceRef>agentTask</bpmn:sourceRef><bpmn:targetRef>Task_RunAgent_Input_agentTask</bpmn:targetRef></bpmn:dataInputAssociation>
      <bpmn:dataOutputAssociation><bpmn:sourceRef>Task_RunAgent_Output_agentResult</bpmn:sourceRef><bpmn:targetRef>agentResult</bpmn:targetRef></bpmn:dataOutputAssociation>
