...
use crate::executors::wendao_sql::input::required_context_string;
use crate::executors::wendao_sql::render::validation_report_xml;
...
impl QianjiMechanism for WendaoSqlValidateMechanism {
    async fn execute(&self, context: &serde_json::Value) -> Result<QianjiOutput, String> {
        ...
        let bundle = crate::executors::wendao_sql::contract::parse_surface_bundle_xml(
            surface_bundle_raw.as_str(),
        )?;
        let spec = crate::executors::wendao_sql::contract::parse_sql_author_spec_xml(
            author_spec_raw.as_str(),
        )?;
