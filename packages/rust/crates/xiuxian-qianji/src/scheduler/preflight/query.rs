...
pub(crate) fn resolve_dynamic_query_with_uri_expansion(
    ...
    {
        let _ = query_expression;
        return Ok(None);
    }

    #[cfg(feature = "wendao-integration")]
    {
        let mut uris = embedded_discover_canonical_uris(query_expression).map_err(|error| {
            format!("semantic discovery failed for `{query_expression}`: {error}")
        })?;
        if uris.is_empty() {
            return Ok(None);
        }
