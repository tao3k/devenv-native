...
async fn runtime_transaction_mixed_cancel_timer_boundary_routes_and_cancels_cancel_sibling() {
    ...
    let mut instance = create_instance(
        ...
        BpmnInstanceInit::new(
            "wf_transaction_mixed_cancel_timeout",
            json!({ "amount": 7 }),
            10,
        ),
    )
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_transaction_mixed_cancel_path_rolls_back_and_clears_parent_wait() {
    let package = Arc::new(parsed_fixture_package(
        "transaction-mixed-cancel-boundaries.bpmn",
    ));
    let mut instance = create_instance(
        ...
        "main_process",
        BpmnInstanceInit::new(
            "wf_transaction_mixed_cancel_path",
            json!({ "amount": 7 }),
            10,
        ),
    )
    ...
    assert_eq!(
        instance.node_states[node_index(&package, "main_process", "cancelled_review") as usize]
            .status,
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_transaction_mixed_cancel_success_clears_both_boundaries() {
    let package = Arc::new(parsed_fixture_package(
        "transaction-mixed-cancel-boundaries.bpmn",
    ));
    let mut instance = create_instance(
        ...
        "main_process",
        BpmnInstanceInit::new(
            "wf_transaction_mixed_cancel_success",
            json!({ "amount": 7 }),
            10,
        ),
    )
