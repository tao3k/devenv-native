...
fn assert_parent_timer_boundary_wait_armed(
    ...
    assert_eq!(
        ...
        Some(node_index(package, "main_process", "inline_review"))
    );
    assert_eq!(
...
async fn runtime_embedded_subprocess_mixed_timer_boundary_routes_and_cancels_error_siblings() {
    ...
    let mut instance = create_instance(
        ...
        BpmnInstanceInit::new(
            "wf_embedded_subprocess_mixed_timeout",
            json!({ "amount": 7 }),
            10,
        ),
    )
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_embedded_subprocess_mixed_error_route_clears_timer_wait() {
    let package = Arc::new(parsed_fixture_package(
        "embedded-subprocess-mixed-boundaries.bpmn",
    ));
    let mut instance = create_instance(
        ...
        "main_process",
        BpmnInstanceInit::new(
            "wf_embedded_subprocess_mixed_error",
            json!({ "amount": 7 }),
            10,
        ),
    )
    ...
    let completion = advance_instance(package.as_ref(), &mut instance, &host)
        .await
        .must("embedded subprocess error end should route through matching error boundaries");
    assert_eq!(completion, BpmnAdvanceOutcome::Completed);
    assert_eq!(instance.lifecycle, InstanceLifecycle::Completed);
    assert_eq!(instance.process.process_id.as_ref(), "main_process");
    assert!(instance.call_stack.is_empty());
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_embedded_subprocess_mixed_success_clears_all_boundaries() {
    let package = Arc::new(parsed_fixture_package(
        "embedded-subprocess-mixed-boundaries.bpmn",
    ));
    let mut instance = create_instance(
        ...
        "main_process",
        BpmnInstanceInit::new(
            "wf_embedded_subprocess_mixed_success",
            json!({ "amount": 7 }),
            10,
        ),
    )
    ...
    let completion = advance_instance(package.as_ref(), &mut instance, &host)
        .await
        .must("embedded subprocess should complete normally and cancel sibling boundaries");
    assert_eq!(completion, BpmnAdvanceOutcome::Completed);
    assert_eq!(instance.lifecycle, InstanceLifecycle::Completed);
    assert_eq!(instance.process.process_id.as_ref(), "main_process");
    assert!(instance.call_stack.is_empty());
