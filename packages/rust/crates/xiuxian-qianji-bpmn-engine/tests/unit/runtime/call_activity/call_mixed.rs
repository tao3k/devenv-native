...
fn assert_timeout_route_open(
    ...
    assert_eq!(
        instance.node_states[node_index(package, "main_process", "review_error_specific") as usize]
            .status,
        NodeRuntimeStatus::Cancelled
    );
    assert_eq!(
...
async fn runtime_call_activity_mixed_timer_boundary_routes_and_cancels_error_siblings() {
    let package = Arc::new(parsed_fixture_package(
        "call-activity-mixed-boundaries.bpmn",
    ));
    let mut instance = create_instance(
        Arc::clone(&package),
        "main_process",
        BpmnInstanceInit::new("wf_call_activity_mixed_timeout", json!({ "amount": 7 }), 10),
    )
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_call_activity_mixed_error_route_clears_timer_wait() {
    let package = Arc::new(parsed_fixture_package(
        "call-activity-mixed-boundaries.bpmn",
    ));
    let mut instance = create_instance(
        Arc::clone(&package),
        "main_process",
        BpmnInstanceInit::new("wf_call_activity_mixed_error", json!({ "amount": 7 }), 10),
    )
    .must("instance should be created");
    let host = StubHost::new(55);
    ...
    let completion = advance_instance(package.as_ref(), &mut instance, &host)
        .await
        .must("call activity error end should route through matching error boundaries");
    assert_eq!(completion, BpmnAdvanceOutcome::Completed);
    assert_eq!(instance.lifecycle, InstanceLifecycle::Completed);
    assert_eq!(instance.process.process_id.as_ref(), "main_process");
    assert!(instance.call_stack.is_empty());
...
#[tokio::test(flavor = "current_thread")]
async fn runtime_call_activity_mixed_success_clears_all_boundaries() {
    let package = Arc::new(parsed_fixture_package(
        "call-activity-mixed-boundaries.bpmn",
    ));
    let mut instance = create_instance(
        Arc::clone(&package),
        "main_process",
        BpmnInstanceInit::new("wf_call_activity_mixed_success", json!({ "amount": 7 }), 10),
    )
    .must("instance should be created");
    let host = StubHost::new(55);
    ...
    let completion = advance_instance(package.as_ref(), &mut instance, &host)
        .await
        .must("call activity should complete normally and cancel sibling boundaries");
    assert_eq!(completion, BpmnAdvanceOutcome::Completed);
    assert_eq!(instance.lifecycle, InstanceLifecycle::Completed);
    assert_eq!(instance.process.process_id.as_ref(), "main_process");
    assert!(instance.call_stack.is_empty());
