...
`xiuxian-qianji-bpmn-engine` currently accepts and how they map onto the runtime
host seam.
...
- Supported host-dispatched tasks may carry bounded native BPMN task Data/IO:
  ...
  variable mappings. Dotted output targets are merged into existing workflow
  objects so repeated completions can fill sibling fields without erasing
