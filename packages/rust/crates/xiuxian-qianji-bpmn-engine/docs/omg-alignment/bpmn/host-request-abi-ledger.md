...
1. `xiuxian-qianji-bpmn-engine` owns pending work identity, form metadata, assignment
...
When multiple host completions write different fields under the same dotted
`targetRef` object, the runtime deep-merges those mapped objects instead of
replacing earlier sibling fields.
