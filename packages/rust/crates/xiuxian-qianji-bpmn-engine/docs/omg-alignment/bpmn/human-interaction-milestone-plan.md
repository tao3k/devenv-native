...
- `xiuxian-qianji-bpmn-engine` owns parsing, linting, runtime state, pending host-work
...
| ----- | ------------------------------- | ------------------------------------------------------------ | -------------------------------------- |
| 1     | M1 Host Loop Conformance        | Establishes the runtime shape before adding more metadata    | `xiuxian-qianji-bpmn-engine`                   |
| 2     | M2 Host Request ABI Ledger      | Prevents field drift across stream, HTTP, CLI, and adapters  | `xiuxian-qianji-bpmn-engine`, `xiuxian-qianji` |
| 3     | M4 Native Rendering Boundary    | Closes standard rendering ambiguity before UI work resumes   | `xiuxian-qianji-bpmn-engine` linter            |
| 4     | M5 Assignment Boundary          | Keeps routing metadata and authorization semantics separated | `xiuxian-qianji-bpmn-engine`, `xiuxian-qianji` |
| 5     | M6 Identity and Claim Lifecycle | Hardens durable operator coordination                        | `xiuxian-qianji-bpmn-engine`, `xiuxian-qianji` |
| 6     | M3 Task Data Shape              | Requires stable ABI and completion evidence first            | `xiuxian-qianji-bpmn-engine`                   |
| 7     | M7 Worklist Routing Policy      | Depends on assignment and claim boundaries                   | `xiuxian-qianji`                       |
| 8     | M8 Form Schema Boundary         | Expands only after current form ABI is locked                | `xiuxian-qianji-bpmn-engine` linter/runtime    |
| 9     | M9 Global Human Task Policy     | Lower frequency; should follow core task stability           | `xiuxian-qianji-bpmn-engine` parser/linter     |
