...
pub(super) fn issue_from_bpmn_execution_shape_error(error: &BpmnEngineError) -> Option<LintIssue> {
    Some(match error {
        BpmnEngineError::UnsupportedMultipleEventDefinitions {
            ...
            process_id,
            node_id,
        } => multiple_event_definition_issue(
            Some(source_id),
            process_id,
            node_id,
            "multiple_event_definitions",
        ),
        BpmnEngineError::UnsupportedLoopConfiguration {
            process_id,
            node_id,
            detail,
        } => loop_configuration_issue(process_id, node_id, detail),
        ...
        BpmnEngineError::UnsupportedEventConfiguration {
            ...
            *detail,
            "multiple_event_definition_deferred" | "parallel_multiple_event_definition_deferred"
        ) => multiple_event_definition_issue(None, process_id, node_id, detail),
        BpmnEngineError::UnsupportedEventConfiguration {
            process_id,
            node_id,
            detail,
...
fn multiple_event_definition_issue(
    ...
    LintIssue::from_parts(
        ...
        format!(
            "Process '{process_id}' event node '{node_id}' uses unsupported multiple-event configuration '{detail}'."
        ),
        "The current event milestone supports one concrete event definition per start, catch, throw, boundary, or end event. BPMN `multipleEventDefinition` and `parallelMultipleEventDefinition`, or several concrete event definitions on one node, require a separate fan-in/fan-out runtime policy.",
        vec![
            "Rewrite the event as one concrete event definition such as `messageEventDefinition`, `signalEventDefinition`, `timerEventDefinition`, `conditionalEventDefinition`, `errorEventDefinition`, `escalationEventDefinition`, `cancelEventDefinition`, or `terminateEventDefinition` where that event kind is already supported.".to_string(),
            "If the workflow needs multiple alternatives, model the choice explicitly with supported event-based gateway waits or separate boundary events instead of relying on one multiple-event node.".to_string(),
        ],
        format!(
            "Repair event node '{node_id}' in process '{process_id}' by replacing unsupported multiple-event configuration '{detail}' with one supported concrete event definition, or remodel the alternatives through an explicit supported event-based gateway or boundary-event structure. Preserve BPMN ids where possible."
        ),
        json!({
            "source_id": source_id,
            "process_id": process_id,
