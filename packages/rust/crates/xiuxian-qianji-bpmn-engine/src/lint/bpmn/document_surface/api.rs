use super::evidence::{
    ...
    diagram_required_attribute_issue, diagram_topology_issue, flow_element_metadata_issue,
    io_set_lifecycle_issue, issue_for_tag, local_name, resource_role_metadata_issue,
};
...
pub(in crate::lint::bpmn) fn deferred_document_surface_issue(
    ...
    if let Some(issue) = io_set_lifecycle_issue(source) {
        return Some(issue);
    }
    if let Some(issue) = diagram_namespace_issue(source) {
        return Some(issue);
    }
    if let Some(issue) = diagram_required_attribute_issue(source) {
        return Some(issue);
    }
