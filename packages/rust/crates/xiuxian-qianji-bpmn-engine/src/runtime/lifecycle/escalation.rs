...
pub(crate) fn escalation_subprocess_shell(
    package: &BpmnPackage,
    ...
    let frame = pop_call_activity_frame(instance).ok_or(BpmnEngineError::UnsupportedOperation {
        operation: "escalation_subprocess_shell_missing_parent_frame",
    })?;
    ...
    let subprocess_node = process.nodes.get(return_node_index as usize).ok_or(
        BpmnEngineError::UnsupportedOperation {
            operation: "escalation_subprocess_shell_missing_parent_node",
        },
    )?;
    if !matches!(
        ...
        return Err(BpmnEngineError::UnsupportedOperation {
            operation: "escalation_subprocess_shell_parent_not_supported_shell",
        });
    }
    ...
    let matching_boundaries = process
        ...
        .filter_map(|boundary| {
            ...
            (boundary_event.kind == BpmnEventKind::Escalation
                && escalation_reference_matches(
                    thrown_reference_id,
    ...
    if matching_boundaries.is_empty() {
        return Err(BpmnEngineError::UnsupportedOperation {
            operation: "escalation_subprocess_shell_missing_boundary",
        });
    }

    let parent_token_index = state::token_index_for_node(instance, return_node_index).ok_or(
        BpmnEngineError::UnsupportedOperation {
            operation: "escalation_subprocess_shell_missing_parent_token",
        },
    )?;
    ...
    for boundary_index in &matching_boundaries {
        ...
        let edge_index = state::resolve_single_outgoing_edge(
            process,
            *boundary_index,
            "escalation_subprocess_shell_routing",
        )?;
    ...
    let Some((first_edge_index, first_next_node_index)) = selected_routes.first().copied() else {
        return Err(BpmnEngineError::UnsupportedOperation {
            operation: "escalation_subprocess_shell_missing_boundary_route",
        });
    };
...
fn escalation_reference_matches(
    thrown_reference_id: Option<&str>,
