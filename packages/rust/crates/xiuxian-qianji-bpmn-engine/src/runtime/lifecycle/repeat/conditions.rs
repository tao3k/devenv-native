...
pub(crate) fn merge_output_data_excluding(
    ...
    if let Some(obj) = output_data.as_object() {
        for (key, value) in obj {
            ...
            merge_value(&mut variables[key], value);
        }
    }
}

fn merge_value(target: &mut serde_json::Value, value: &serde_json::Value) {
    ...
    for (key, nested_value) in value_object {
        let target_entry = target_object
            .entry(key.clone())
            .or_insert(serde_json::Value::Null);
        merge_value(target_entry, nested_value);
    }
}
