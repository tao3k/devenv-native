...
fn dataset_ontology_materialize_cache_key(
    route: &str,
    metadata: &tonic::metadata::MetadataMap,
) -> Result<String, Status> {
    let manifest = validate_dataset_ontology_materialize_request_metadata(metadata)?;
    let table_payloads = manifest
        .tables
        .iter()
        .map(|table| format!("{}={}", table.table_name, table.payload_id))
        .collect::<Vec<_>>()
        .join(",");
    Ok(format!(
...
impl WendaoFlightService {
    ...
    async fn read_attachment_search_payload(
        ...
        let provider = self.attachment_search_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "attachment-search Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_ast_search_payload(
        ...
        let provider = self.ast_search_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "AST-search Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_definition_payload(
        ...
        let provider = self.definition_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "definition Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_autocomplete_payload(
        ...
        let provider = self.autocomplete_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "autocomplete Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_sql_payload(
        ...
        let provider = self.sql_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "SQL Flight route `{route}` is not configured for this transport host"
            ))
        })?;
    ...
    async fn read_vfs_resolve_payload(
        ...
        let provider = self.vfs_resolve_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "VFS resolve Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_vfs_content_payload(
        ...
        let provider = self.vfs_content_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "VFS content Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_graph_neighbors_payload(
        ...
        let provider = self.graph_neighbors_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "graph-neighbors Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_vfs_scan_payload(&self, route: &str) -> Result<FlightRoutePayload, Status> {
        let provider = self.vfs_scan_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "VFS scan Flight route `{route}` is not configured for this transport host"
            ))
        })?;
    ...
    async fn read_topology_3d_payload(&self, route: &str) -> Result<FlightRoutePayload, Status> {
        let provider = self.topology_3d_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "topology-3d Flight route `{route}` is not configured for this transport host"
            ))
        })?;
    ...
    async fn read_markdown_analysis_payload(
        ...
        let provider = self.markdown_analysis_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "markdown analysis Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_code_ast_analysis_payload(
        ...
        let provider = self.code_ast_analysis_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "code-AST analysis Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_repo_overview_payload(
        ...
        let provider = self.repo_overview_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "repo overview Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_repo_index_payload(
        ...
        let provider = self.repo_index_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "repo index Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_repo_index_status_payload(
        ...
        let provider = self.repo_index_status_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "repo index status Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_repo_sync_payload(
        ...
        let provider = self.repo_sync_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "repo sync Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_repo_doc_coverage_payload(
        ...
        let provider = self.repo_doc_coverage_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "repo doc coverage Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_repo_projected_page_index_tree_payload(
        ...
        let provider = self
            ...
            .ok_or_else(|| {
                Status::unimplemented(format!(
                    "repo projected page-index tree Flight route `{route}` is not configured for this transport host"
                ))
            })?;
        provider
    ...
    async fn read_repo_projected_retrieval_context_payload(
        ...
        let provider = self
            ...
            .ok_or_else(|| {
                Status::unimplemented(format!(
                    "repo projected retrieval-context Flight route `{route}` is not configured for this transport host"
                ))
            })?;
        provider
    ...
    async fn read_refine_doc_payload(
        ...
        let provider = self.refine_doc_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "refine-doc Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_document_extract_payload(
        ...
        let provider = self.document_extract_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "document extract Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
    ...
    async fn read_dataset_ontology_materialize_payload(
        &self,
        route: &str,
        metadata: &tonic::metadata::MetadataMap,
    ) -> Result<FlightRoutePayload, Status> {
        ...
        let provider = self
            ...
            .ok_or_else(|| {
                Status::unimplemented(format!(
                    "dataset ontology materialize Flight route `{route}` is not configured for this transport host"
                ))
            })?;
        provider
    ...
    async fn read_search_family_payload(
        ...
        let provider = self.search_provider.as_ref().ok_or_else(|| {
            Status::unimplemented(format!(
                "search Flight route `{route}` is not configured for this transport host"
            ))
        })?;
        provider
            .search_batch(
