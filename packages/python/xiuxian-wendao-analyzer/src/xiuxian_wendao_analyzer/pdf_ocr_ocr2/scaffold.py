"""Hosted VLM/OCR structural scaffold sidecar validation and canonicalization."""
...
_HOSTED_VLM_OCR_REGION_SCAFFOLD_FILE_NAME = "_hosted_vlm_region_scaffolds.json"
_HOSTED_VLM_OCR_REGION_SCAFFOLD_SCHEMA = "xiuxian_wendao.hosted_vlm_region_scaffold.v1"
...
def resolve_ocr2_region_scaffold_sidecar_path(image_path: Path) -> Path:
    direct_path = image_path.parent / _HOSTED_VLM_OCR_REGION_SCAFFOLD_FILE_NAME
    for depth, directory in enumerate(image_path.parents):
        if depth >= 6:
            break
        candidate = directory / _HOSTED_VLM_OCR_REGION_SCAFFOLD_FILE_NAME
...
def read_ocr2_region_scaffold_sidecar(path: Path) -> Mapping[str, Any]:
    if not path.is_file():
        raise ValueError(f"missing Hosted VLM/OCR region scaffold sidecar: {path}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError(
            f"malformed Hosted VLM/OCR region scaffold sidecar: {path}"
        ) from exc
    if not isinstance(payload, Mapping):
        raise ValueError("Hosted VLM/OCR region scaffold sidecar is not an object")
    if payload.get("schema") != _HOSTED_VLM_OCR_REGION_SCAFFOLD_SCHEMA:
        raise ValueError(
            "Hosted VLM/OCR region scaffold sidecar has unsupported schema"
        )
    items = payload.get("items")
    if not isinstance(items, list):
        raise ValueError("Hosted VLM/OCR region scaffold sidecar is missing items")
    return payload
...
def match_ocr2_region_scaffold(
    ...
    if len(matches) != 1:
        raise ValueError(
            "Hosted VLM/OCR region scaffold item count does not match input row"
        )
    item = matches[0]
    checks = [
    ...
    for item_key, row_key in checks:
        if str(item.get(item_key) or "") != str(input_row.get(row_key) or ""):
            raise ValueError(
                f"Hosted VLM/OCR region scaffold fingerprint mismatch for {item_key}"
            )
    if int(item.get("pageIndex", -1)) != int(input_row.get("pageIndex", -2)):
        raise ValueError("Hosted VLM/OCR region scaffold page index mismatch")
    if int(item.get("regionIndex", -1)) != int(input_row.get("regionIndex", -2)):
        raise ValueError("Hosted VLM/OCR region scaffold region index mismatch")
    return item
...
def extract_ocr2_scaffold_markdown(
    ...
    if not response_text.strip():
        raise ValueError("Hosted VLM/OCR scaffold response returned empty text")
    try:
        payload = json.loads(response_text)
    except json.JSONDecodeError as exc:
        raise ValueError("Hosted VLM/OCR scaffold response is not valid JSON") from exc
    if not isinstance(payload, Mapping):
        raise ValueError("Hosted VLM/OCR scaffold response is not a JSON object")
    regions = payload.get("regions")
    if not isinstance(regions, list):
        raise ValueError("Hosted VLM/OCR scaffold response is missing regions")
    if len(regions) != len(input_rows):
        raise ValueError("Hosted VLM/OCR scaffold response row count mismatch")
    markdown_rows: list[str] = []
    for region, input_row in zip(regions, input_rows, strict=True):
        if not isinstance(region, Mapping):
            raise ValueError("Hosted VLM/OCR scaffold region is not an object")
        expected_marker = ocr2_region_marker(input_row)
        if str(region.get("marker") or "") != expected_marker:
            raise ValueError("Hosted VLM/OCR scaffold response marker mismatch")
        if str(region.get("shardElementId") or "") != str(
            input_row.get("shardElementId") or ""
        ):
            raise ValueError("Hosted VLM/OCR scaffold response shard id mismatch")
        markdown = canonicalize_ocr2_scaffold_region_markdown(region)
        if not markdown.strip():
            raise ValueError(
                "Hosted VLM/OCR scaffold response returned empty canonical text"
            )
...
def canonicalize_ocr2_scaffold_region_markdown(region: Mapping[str, Any]) -> str:
    ...
    if tables is not None:
        if not isinstance(tables, list):
            raise ValueError("Hosted VLM/OCR scaffold tables field is not a list")
        for table in tables:
            if not isinstance(table, Mapping):
                raise ValueError("Hosted VLM/OCR scaffold table is not an object")
...
def canonical_markdown_table(rows: Any) -> str:
    if not isinstance(rows, list) or not rows:
        raise ValueError("Hosted VLM/OCR scaffold table rows are empty")
    canonical_rows: list[list[str]] = []
    expected_width: int | None = None
    for row in rows:
        if not isinstance(row, list) or not row:
            raise ValueError("Hosted VLM/OCR scaffold table row has invalid cell shape")
        cells = [canonical_markdown_cell(cell) for cell in row]
        if expected_width is None:
            expected_width = len(cells)
        elif len(cells) != expected_width:
            raise ValueError(
                "Hosted VLM/OCR scaffold table rows have inconsistent cell shape"
            )
