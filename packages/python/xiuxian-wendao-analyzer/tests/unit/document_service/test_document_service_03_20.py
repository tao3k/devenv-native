...
def _write_sparse_png(path: Path) -> None:
...
def test_docling_pdf_ocr_worker_hedges_slow_region_composite_request(
    tmp_path: Path,
    monkeypatch,
) -> None:
    source = tmp_path / "source.pdf"
    ...
    for index, image in enumerate(region_images):
        image.write_bytes(f"region png fixture {index}".encode())
    trace_path = tmp_path / "hosted-vlm-region-composite-hedged.jsonl"
    requests: list[object] = []
    ...
    assert [row["text"] for row in table.to_pylist()] == ["fast A", "fast B"]
    assert len(requests) == 2
    records = [
        json.loads(line) for line in trace_path.read_text(encoding="utf-8").splitlines()
    ]
    assert records[0]["requestKind"] == "region-composite-hedged"
    assert records[0]["httpAttemptCount"] == 2
    assert records[0]["shardCount"] == 2


def test_docling_pdf_ocr_worker_trims_region_composite_payload(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from PIL import Image
    ...
    for image in region_images:
        _write_sparse_png(image)
    trace_path = tmp_path / "hosted-vlm-region-composite-trimmed.jsonl"
    requests: list[object] = []
    ...
    monkeypatch.setenv(HOSTED_VLM_OCR_REGION_COMPOSITE_SIZE_ENV, "2")
    monkeypatch.setenv(HOSTED_VLM_OCR_TRACE_PATH_ENV, str(trace_path))
    monkeypatch.setenv(HOSTED_VLM_OCR_IMAGE_OPTIMIZATION_ENV, "region-whitespace-trim")
    monkeypatch.setattr(
        "xiuxian_wendao_analyzer.pdf_ocr_ocr2.http.urllib.request.urlopen",
        fake_urlopen,
    )
    ...
    assert [row["text"] for row in table.to_pylist()] == ["trimmed A", "trimmed B"]
    payload = json.loads(requests[0].data.decode("utf-8"))
    sent_pngs = []
    ...
    assert len(sent_pngs) == 2
    records = [
        json.loads(line) for line in trace_path.read_text(encoding="utf-8").splitlines()
    ]
    assert records[0]["imageBytes"] == sum(len(sent_png) for sent_png in sent_pngs)
